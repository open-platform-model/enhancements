...
Defer to `enhancements` skill `## Phase 2 — Iterate`. The block shape is fixed: `**Kind:**`, then the optional relation fields (`**Depends:**`, `**Amends:**`, `**Supersedes:**`, `**Resolves:**`, `**Revised:**`; `Amends` / `Supersedes` tokens are bare `DN` for this entry's decisions and qualified `MMMM:DN` for another entry's), then the four body fields **Decision**, **Alternatives considered**, **Rationale**, **Source**. Reproduce that shape in the auto-draft. Do not deviate. Do not invent fields outside it.
