...
Defer to `enhancements` skill `## Phase 2 — Iterate`. The block shape is fixed: `**Kind:**`, then the optional relation fields (`**Depends:**`, `**Amends:**`, `**Supersedes:**`, `**Resolves:**`, `**Revised:**`; `Amends` / `Supersedes` tokens are bare `DN` for this entry's decisions and qualified `MMMM:DN` for another entry's), then the body fields **Decision**, **Requirements** (numbered `Rn` items, or `none ({reason})` on a `policy` or `scope` decision), **Alternatives considered**, **Rationale**, **Source**. Reproduce that shape in the auto-draft. Do not deviate. Do not invent fields outside it.
...
- **Renumbering or reordering decisions — or rewriting bodies on an `accepted` entry.** `D1`, `D2`, … are stable identifiers that other repos cite; never reuse or renumber. On a `draft`, the walk may revise an existing `### DN:` block in place when the OQ's resolution changes that decision (fold discipline applies). On an `accepted` entry the walk only ever *inserts* new blocks with relation fields — editing an existing body there is the `enhancement-compaction` skill's job, with its own manifest and its own commit. Doing it mid-walk hides a rewrite inside an unrelated change.
