...
2. **Change refs are structured, never paths and never URLs.** Paths break when an OpenSpec change is archived (the directory moves); URLs rot. The (repo, key) pair is permanent:
   - `{kind: openspec, repo: <repo>, change: <slug>}`, slug **without** the archive date prefix, so the ref survives the `changes/` to `changes/archive/YYYY-MM-DD-<slug>` move.
   - `{kind: pr, repo: <repo>, number: N}` for repos without an OpenSpec workspace.
   - `{kind: commit, repo: <repo>, sha: <sha>}` for work landing without a PR; log only after merge, since rebases invalidate pre-merge shas.
   - `{kind: retrospective, repo: <repo>, note: "..."}` strictly for pre-tracking landings, where per-change archaeology would invent precision the record never had. Never for new work.
