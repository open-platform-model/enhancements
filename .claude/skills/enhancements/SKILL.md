...
- **Recording a mechanism decision.** If it would not bind a from-scratch rewrite of the affected repos, it is not a `DN` — it belongs in the implementing slice's OpenSpec change. `task check` flags path and identifier references in an entry's prose as smells.
