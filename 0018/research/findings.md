...
- **From the scaffold disagreement, the no-placeholder rule.** D8 lets a section appear only once it has a real page. The initial inventory in the design is a sizing aid for exactly that reason. Reversed 2026-09-26: every page is built from the moment it exists while the site is unpublished, and what a page must hold before publication is OQ15.
