...
Six gates must all hold before promoting this design from draft to accepted:
...
- **The declaration is exercised as an aspect.** `examples.cue` attaches the `offering` module trait on an aspect of the offered module and pins the declared group and kind against the definition's, so the dependency on entry 0025's aspect map is proved rather than assumed.
