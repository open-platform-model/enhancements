...
One platform-owned object binds a module lineage, a release and an update policy, and a consumer supplies values against the module's own configuration schema. A second layer serves that object as a Kubernetes kind, so the schema reaches the API server and access control can name what a team may create. Both are projections onto the render path OPM already has.

This entry rests on entry 0025, which gives a module one named place to state facts about itself as a whole: aspects, bundles of catalog-published module traits (0025:D11, 0025:D12). The offering declaration a module makes here is one such trait on one such aspect, and nothing else in this design touches the aspect map.
...
- **The module-scoped extension point itself.** What a module may say about itself, and how the kernel matches and renders it, is entry 0025. This entry publishes one module trait against that vocabulary (D11) and adds nothing to it.

## High-Level Approach

Two layers. The first is a binding; the second serves the binding as a kind and is a pure projection onto the first (D4). Above both sits the module's own declaration of what it is when offered, which is an aspect under entry 0025 and is read, never rendered.
...
- **`#ModuleTrait`** (mirror, 0025:D12): the `offering` trait D11 publishes is one of these. Its spec carries the declared group and kind, a suggested update policy and, later, a status schema (OQ5).
- **`#ModuleInstance`**: unchanged. The operator's `ModuleInstance` CR gains an optional definition reference as an alternative to its module reference; that CR field is operator surface, not core.

## Affected Surfaces

**core.** Three new definitions: the definition shape, the served-instance shape and the projection. `#Module`, `#ModuleInstance` and the aspect definitions entry 0025 adds do not change here. The projection is the contract every frontend computes. SPEC.md gains the constructs; a `core-schema-edit` co-update.

**library.** The kernel reads the projection off a built value and renders the result through the render path unchanged, and answers "does this module's `#config` encode to a structural schema" so the refusal in D2 has one implementation the CLI and the operator share. Kernel neutrality holds: no Kubernetes client, no CRD install; the encoder is a pure function over CUE.
