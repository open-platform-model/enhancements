...
- **The declaration's fulfilment is another entry's answer.** The `offering` trait is consumed outside the render path, and how such a trait states `fulfilment` is entry 0025's OQ13. **Mitigation:** the trait ships with whatever 0025 decides, exactly as entry 0009's lifecycle trait does, and nothing here re-decides it. The coupling is a promotion-order constraint, not a design risk: this entry cannot reach `accepted` on a question its substrate has not answered.
