...
{Three personas at most. Each story names a role, a goal, and the blocker
today. Keep stories grounded in roles that exist in the OPM ecosystem
(platform team operator, application module author, catalog author, kernel
contributor, etc.).}

- As a {role}, I want to {goal} so that {outcome}. Today: {blocker}.
- As a {role}, I want to {goal} so that {outcome}. Today: {blocker}.

## Why Existing Workarounds Fail

{If there are current workarounds (hardcoding, manual steps, duplication),
explain why they are insufficient. What do they cost in maintenance,
correctness, or usability? Why is a proper solution needed instead?}
