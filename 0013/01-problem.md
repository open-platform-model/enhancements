...
OPM models a sensitive field as a contract type instead of the scalar it is, and the routing that type carries is now duplicated by hand with nothing checking it. Every claim below names the file it comes from, at commits `core 7500c5d` and `catalog_opm db05149`.
