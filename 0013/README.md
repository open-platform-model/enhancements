...
The design rests on one split. The **attribute** carries routing: which Kubernetes object a key belongs in. That's metadata, written by the module author, identical in every environment. The **type** carries fulfilment: the data itself, or where it already lives. That's data, written by the deployer, different per environment, and type-checked by CUE rather than by OPM tooling.
