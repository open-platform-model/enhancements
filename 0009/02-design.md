...
The kernel grows a second half, a pure planner and orchestrator over four new primitives, layered on the same `#Module` the render half already reads. Trade-off reasoning lives in `03-decisions.md`.
...
**`library/`**: new `opm/flow/` package (planner, runner, `Executor` interface, `Registry`, plan types); `Kernel.PlanLifecycle` / `Kernel.PlanWorkflow` / run entry points; opt-in `opm/helper/executor/*` backend hosts; attribute reading in the planner via `cue.Value.Attribute`. This half also owns the kernel's cancellation path (D9): designed and wired here, untouched by every other change until then. The kernel's former logger, tracer and clock slots were removed as write-only surface (revised D9); this half introduces the injection surface the planner and runner need together with its first reader.
