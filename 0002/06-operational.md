...
- **Cluster:** The API-group/CRD rename (D5) means rollback is a reinstall, mirroring the forward path:

  1. Remove the new CRDs (clear `opmodel.dev/cleanup` finalizers first, so resources don't hang).
  2. Reinstall the old-group CRDs.
  3. Re-apply CRs under `releases.opmodel.dev`.

  Labels revert with the re-apply; SSA reconvergence is clean. Any object selected only by the new key during the window is briefly orphaned from selectors: roll labels and consumers back together.
