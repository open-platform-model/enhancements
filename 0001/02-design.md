...
The realisation is a new `Kernel.Materialize(*Platform) (*MaterializedPlatform, error)` step. For each subscription, it:

1. Parses any `range` string Go-side via `github.com/Masterminds/semver/v3` (D11: CUE cannot evaluate SemVer range syntax natively).
2. Resolves the in-range subset against the OCI registry (via `cuelang.org/go/mod`).
3. Applies `allow` and `deny` per D10's order.
4. Pulls every selected build into the local CUE module cache.
5. Loads each package.
6. Indexes top-level `#ComponentTransformer` values by their stamped FQN.

The result is a synthetic `#composedTransformers: #TransformerMap` plus a `#matchers.{resources,traits}: [FQN]: [...#ComponentTransformer]` reverse index that `Match` consults.

`Match` takes `*MaterializedPlatform` instead of `*Platform`. Materialize is an explicit step rather than implicit-inside-Match, so the caller controls when pulling and indexing happen, typically once per platform spec, reused across many `Match` calls. The kernel itself holds no `MaterializedPlatform` cache (D14: keeps the kernel free of process-level state per `library/CONSTITUTION.md`); a sibling helper package (`library/opm/materialize/cache/`) ships optional cache primitives (interface + reference LRU implementation + spec-content-hash key derivation) that operator / CLI consumers can wire up.

The `opm-operator/` controller is the expected first caller: it owns the natural cache key (the `Platform` CR's `metadata.generation`) and the invalidation moment (CR generation bump). Single-shot `cli/` callers can opt out entirely and pay the materialize cost per invocation; `cuelang.org/go/mod` already maintains an on-disk module cache, so the OCI pull cost amortises across calls even without a kernel-level cache.
...
Catalog-monolithic SemVer is the deliberate design (D18). The catalog is the unit of versioning; primitives inherit. Bumping `id.Version` from `1.4.0` to `1.5.0` bumps every primitive's FQN in lockstep, even ones whose schema didn't change. Two mitigations keep consumers from churning their pins:

1. D6's always-unify means byte-identical primitive bodies across adjacent SemVers unify cleanly. The consumer pinned at `container@1.4.0` matching a platform that materialised `container@1.5.0` with an unchanged body works, because the matcher unifies the consumer's pinned value with the materialised SemVer in the bucket the consumer pinned. It does not silently rewrite their pin upward.
2. `#SubscriptionFilter.range` covers multiple SemVers at materialize time, so a platform on `>=1.0.0 <2.0.0` keeps `1.4.0`, `1.4.1`, `1.5.0` all pulled. The consumer's `container@1.4.0` pin keeps matching until the range moves past it.

Per-primitive SemVer (each primitive carrying its own author-maintained version independent of the catalog publish) is explicitly not introduced.
...
Match runs against the materialized platform with FQN-keyed lookup followed by an always-on `unify(consumer_component.#resources[FQN], transformer.requiredResources[FQN])` (and the analogous traits step) before predicate evaluation. There is no `--strict` mode, no skip in production. Unification cost is bounded (a few CUE evaluations per matched pair) and catches the failure mode that would otherwise propagate to render time as a confusing error.
...
`#ReleaseIdentity` carries the deployment-scoped facts that no other slot owns: release name, namespace, UUID, and the cluster-domain default. The kernel populates it once per release via `#ModuleRelease`:
...
There is no `#ContextBuilder` and no builder unification step. Three things happen automatically, in CUE-evaluation order:

1. `#ModuleRelease` sets `#module.#ctx.release` from its own metadata.
2. CUE evaluates every `#Component.#names` against its injected `#release`.
3. The `#ctx.components` comprehension projects each `#names` into a sibling map under `#ctx`.

The matcher and renderer see fully concrete values.
