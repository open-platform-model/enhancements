...
- Catalog-side `catalog.cue` root file convention. Declares `Catalog: #CatalogIdentity` (the identity stamp; D7) plus `#Transformers: [#FQNType]: #ComponentTransformer` (the kernel's sole export-discovery surface; D15) — manifests are explicit, no recursive package walk, no auto-discovery. Source-tree default `Catalog.Version: string | *"0.0.0-dev"` keeps `cue vet` cheap during development; the publish task overwrites with the concrete SemVer in a temp build dir before `cue mod publish`. OCI artifacts ship fully concrete; the kernel never injects a version at load time.
...
| `library/modules/opm/` (CUE module `opmodel.dev/catalogs/opm@v0`) | Catalog source — repackaged to drop any `#Module.#defines` wrapper, source `metadata.version` / `metadata.modulePath` from a new root `Catalog` constant, and add a new root `catalog.cue` file declaring `Catalog: #CatalogIdentity` plus `#Transformers: [#FQNType]: #ComponentTransformer` manifest (the kernel's sole discovery surface; D15). |
| `library/modules/opm/catalog.cue` *(new)* | The manifest file. One per catalog package; assembles `#Transformers` from imports of the subpackages and is the only file the kernel reads at materialize time to discover transformers. |
...
| `modules/Taskfile.yml` | Publish flow — gains the temp-build-dir stamping step that overwrites `Catalog.Version` before `cue mod publish`. |
