...
- New `#Platform.#registry` shape: `[Id=#NameType]: #Subscription`, where a `#Subscription` carries `path` (catalog CUE module path), `enable`, and an optional `filter` (SemVer `range` plus `allow` / `deny` overrides).
...
- Catalog-side `Catalog: { Version, ModulePath }` constant convention. Source-tree default `Version: string | *"0.0.0-dev"` keeps `cue vet` cheap during development; the publish task overwrites with the concrete SemVer in a temp build dir before `cue mod publish`. OCI artifacts ship fully concrete; the kernel never injects a version at load time.
...
| `catalog/opm/v1alpha1/` | Catalog source — repackaged to drop any `#Module.#defines` wrapper, export primitives at top level, and source `metadata.version` / `metadata.modulePath` from a new root `Catalog` constant. |
