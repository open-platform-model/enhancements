...
- **`cli/` rewrite carved into a separate enhancement.** The `accepted → implemented` graduation text scoped the `cli/` kernel rewrite as a 0001 slice. It landed instead as enhancement [0006](../0006/)'s kernel-adoption strand (C2, D9), following the enhancements skill's "carve remaining work into a new enhancement" rule rather than leaving 0001 in perpetual `in-progress`. 0001's graduation gate was amended (2026-06-22) to track the CLI criterion in 0006; it closed when that slice landed on 2026-07-20 (cli `1.0.0-alpha.4`). Design intent — the CLI renders through the library kernel instead of its own `pkg/render/` fork — is preserved unchanged; only the tracking home moved.

The umbrella is now `implemented` (2026-07-20). Every producer-side slice shipped — `core/` schema (`opmodel.dev/core@v0`), `library/` kernel (v0.3.0+), the repackaged catalog (`opmodel.dev/catalogs/opm@v0`), and the complete `opm-operator/` rewrite onto the kernel (2026-06-12). The final consumer gate — the `cli/` rewrite onto the kernel — landed via 0006's kernel-adoption slice on 2026-07-20; verified in `cli/`: `go.mod` requires `github.com/open-platform-model/library`, the `pkg/render/` fork is deleted, 13 Go files import `open-platform-model/library/opm`, and render / materialize / compile / handoff-verify all flow through `kernel.New` → `Compile` / `Materialize`. The workspace `modules/*` mass-rewire (D23, non-blocking wave) is consumer maintenance on each module's own cadence and never gated graduation. See `04-graduation.md` for the amended `accepted → implemented` criteria.
