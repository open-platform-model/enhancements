...
Almost everything this entry adds is a refusal, and the refusals are the feature. Six, all emitted by the publish command before anything is pushed:
...
4. **`catalog_opm`** (the single first-party catalog after 0010 D47's consolidation; the two absorbed repos' pipelines retire with their v2 lines rather than being cut over): switch `release.yml`'s publish job to the new command, delete the copy-and-stamp task. Catalogs go first because modules build against them, so a module cannot be republished correctly until a conforming catalog exists.
...
6. **Deployment repositories** (outside the workspace; named out of band): **no step.** No re-pin is needed for a coordinate change, because D13 leaves first-party paths where they are and there is none. They pick up new module versions the way they always do, which is an ordinary release rather than a migration step.

Steps 1 and 2 were absent from this list until 2026-08-05, which read as though `cli` were the first mover; the then-current delivery plan (since retired) had the ordering right and its header recorded the discrepancy at the time; the landing record is `delivery.yaml`. Step 5's identity files were likewise claimed here and by 0010 at once: D17 settles that they are 0010's.
...
Neither entry blocks the other, which was always true at the entry level. Either may be designed, sliced and landed first; the intermediate state is valid in either order, and 0010's steps 1–5 have no dependency on this entry at all. Enhancement 0010 records the relationship as a **preference rather than a gate**, and its `06-operational.md` separates the two claims under *Where enhancement 0011 binds, and where it does not*: that section's fleet-republish half is superseded by D17 and should be read alongside it.
