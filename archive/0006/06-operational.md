...
- The CLI render pipeline change (D9) is internal, with no user-facing schema change, but render diagnostics and pipeline behaviour shift to the kernel's. The CLI also moves to `cuelang.org/go` v0.17.1 in C1, ahead of the `library` import (D36); accepted under D14.
...
- The `ModuleInstance` CRs written by the CLI are valid operator CRs; a CLI rollback does not strip them. Reverting the CLI binary to a Secret-era version would leave the CRs in place; the operator ignores `spec.owner: cli` ones, so that's harmless. But the old CLI version would not find those CRs, so rollback realistically means re-applying with the old CLI, which recreates Secrets. Because resources carry stable labels/UUIDs and both eras use SSA, re-applying does not churn live objects.
...
Nine slices land in this order (slice 1 was cancelled; see D31). Slice 5 is the one gated on enhancement 0001:
