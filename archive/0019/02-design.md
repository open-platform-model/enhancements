...
- **D28's fail-closed demand resolution survives, with one measured boundary.** Experiment 05: an empty or fully-disqualified bucket yields `missing` and `unresolved` rows as data, and the gate is one unification (`resolved & true`) that refuses the build. Probed through the Go API, `diagnostics` stays fully readable and concrete via `LookupPath` while that gate is failing, so decoding `oerrors.{MissingFQN,UnresolvedDemand,UnifyError}` from a `diagnostics:` field coexists with an in-build refusal (only the CLI conflates them). The boundary is an unhandled trait whose posture is UNSTATED: classifying it evaluates a plain `bool`, which is incomplete. That one refusal therefore arrives as an incomplete-value error naming `core/trait.cue`'s own `optional` field rather than as a diagnostics row. Fail-closed holds either way; making that case data would need a publish-side gate that forbids unstated posture.
...
D18 completes the shape:

1. **Default:** warn-and-render is the default when a caller says nothing, with the policy configurable between warn and refuse only.
2. **Comparison source:** the comparison reads the staged instance module's `cue.mod` against the platform module's tidied list (never the render module's promoted list, which the platform wins by construction, so skew would be invisible there).
3. **Older builds:** a module requiring an older build gets no warning; the resolved-versions comparison rides compile diagnostics as plain data.
...
**What it covers.** The fixture set is enumerated rather than implied:

1. The repaired `TestFlow_WebApp_OnOpmPlatform` instance, five pairs on the real catalog.
2. A `#names` read (`resourceName` and `dns.fqdn`) through a transformer, the D15 shape.
3. A plain and a self-referential `#moduleInstance` read (D3).
4. The ADR-003 hazard shape, an output-local hidden field consumed in-expression.
5. A module assembling its container environment from guarded sources, the D14 ordering case, compared order-sensitively.
6. Experiment 07's two large fixtures in both authoring styles, byte-for-byte.
7. Experiment 05's healthy-plus-sabotaged matching fixtures, once D10's slice moves matching into the build.

Per `#ParityCase.equality`, cases compare `output-fields-only` until D12's projection slice lands and `structural` permanently after.
