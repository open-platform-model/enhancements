...
Substitute the "Release" lexeme with "Instance" across core, library, the operator, and the CLI, with the wire contract and API group moving in the same sequenced rollout. Trade-off reasoning, especially the contested wire-contract questions, lives in `03-decisions.md`.
