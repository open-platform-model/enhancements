...
- `GRAPH.md` — Mermaid relationship diagrams (`depends_on` / `supersedes` / `revives`): a category rollup plus one diagram per `category`, foreign entries drawn as stubs.
...
scripts/                    delivery.sh (derives delivery state from each entry's delivery.yaml), entry_hash.sh (gate binding)
...
GRAPH.md                    Generated relationship diagrams, one per category (run `task graph` after any category / depends_on / supersedes / revives edit)
...
| `task graph` | Regenerate `GRAPH.md`: a category rollup plus one Mermaid diagram per `category`, foreign entries as stubs. Run after any `category` / `depends_on` / `supersedes` / `revives` edit. |

### Workflow phases

```
admit (gates) → new → fill problem + design → accrete decisions → walk gates → promote (accepted) → deliver + log via delivery.yaml
```
...
- **Always load the `enhancements` skill** before doing workflow work (create, edit `config.yaml`, promote status, append history, add `depends_on` / `**Depends:**` edges, run any `task` other than read-only `list`/`show`).
...
- Run `task index` after any `config.yaml` change; `task graph` after any `category` / `depends_on` / `supersedes` / `revives` change.
