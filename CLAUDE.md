...
- `GRAPH.md` — Mermaid relationship diagram (`depends_on` / `supersedes` / `revives`).
...
GRAPH.md                    Generated relationship diagram (run `task graph` after any depends_on / supersedes / revives edit)
...
| Task | Purpose |
| --- | --- |
| `task list` | Status table — id, status, semver, area, derived delivery state, title. First thing to read when picking up unfamiliar work. |
| `task show ID=NNNN` | Full metadata + history list + document list for one entry. |
| `task vet` | **Hard gate** (PR-blocking). CUE schema validation, cross-ref existence, `depends_on` ⇔ `**Depends:**` in both directions (each target a live decision heading, no cycles), placeholder absence, `area ∈ affects`, the `core_schema` rules (`schemas/` iff `core_schema: true`, compiles, `core ∈ affects`, `examples.cue` + `spec.md` from `accepted`), `contracts/` compiles when present, experiments structure when present, `delivery.yaml` when present (schema, DN/OQN refs resolve, `no_work` keys live, not tombstoned, and not also carried). |
...
| `task graph` | Regenerate `GRAPH.md` with a Mermaid relationship diagram. Run after any `depends_on` / `supersedes` / `revives` edit. |
...
- Run `task index` after any `config.yaml` change; `task graph` after any `depends_on` / `supersedes` / `revives` change.
