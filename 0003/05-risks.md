...
- **D9's namespace move is a full-fleet identity change, and it compounds daily.** Owner-scoping (`opmodel.dev/m/<owner>/<name>`) rewrites `metadata.modulePath` for every module already published, and `modulePath` feeds `fqn` → `module.uuid` → `instance.uuid` → the identity label stamped on every rendered resource. So this is not a path redirect with a compatibility shim available; it is a new identity for every module and every live instance of one, and the operator's prune guard and the CLI's inventory both match on the label that changes. Blast radius: every published module, every deployed instance, every pinned reference in `releases/`. **Mitigation:** sequence it early — the cost is proportional to how much is published when it lands (OQ9), so this is the one item where delay is strictly more expensive. Decide adopt-in-place versus recreate before the first slice, and treat the stray artifacts already sharing the namespace (a test fixture at `opmodel.dev/library/testdata/modules/web-app`, legacy `…/v1alpha1` paths) as part of the same inventory rather than a later cleanup.

- **A catalog can still ship a placeholder version, and the failure names nothing.** `#Catalog.metadata.version` defaults to `"0.0.0-dev"`, and every transformer FQN in the catalog inherits it via the pattern constraint. A catalog materialized under a placeholder therefore produces a matcher key space nobody's module can match, and the resulting `no matching transformer` names neither the catalog nor the version. Blast radius: every module rendered against that platform — the failure is total for the affected primitives rather than partial. **Mitigation (D7):** refuse the placeholder at both ends — at publish, and at the materialize pull where the decoded version is compared with the resolved tag. Until OQ13 settles the version source, the invariant continues to rest on a stamping generator outside OPM tooling.
