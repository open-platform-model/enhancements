...
- **Version agreement.** `metadata.version` and the artifact's release tag are the same value, stated in `contracts/contracts.cue` as `#PublishedModuleRef` so a mismatch is a type error rather than a tolerated disagreement.
