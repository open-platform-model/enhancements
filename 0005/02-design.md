...
Generate everything downstream of the Kubernetes OpenAPI: shared types, a generated mirror, generated CRD catalogs, per-resource lifecycle metadata, and an optional composition layer. Design Goals and Non-Goals define the boundary below; trade-off reasoning lives in `03-decisions.md`.

## Design Goals

Six goals, all necessary to close the schema drift and hand-authoring limits `01-problem.md` describes.
...
Each generated resource is stamped with **lifecycle metadata** the operator consumes:

- **Scope**: namespaced or cluster, from discovery.
- **Apply-order phase**: generalizes today's hardcoded `library/pkg/resourceorder` list into per-kind data.
- **Readiness expression**: curated per kind from Kubernetes status conventions, with a `status.conditions[Ready]` fallback.
- **Prune/ownership policy**: ownerReferences plus server-side apply field ownership, now expressible concretely because the target is Kubernetes-only.
...
Six touch points, grouped by repo. File-level targets firm up as decisions land.
