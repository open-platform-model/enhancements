...
OPM gains a first-class side-channel for **extra manifests**: plain YAML and/or a Kustomize directory declared on a release, applied alongside rendered output and owned identically (stamped with OPM ownership labels, recorded in `status.inventory`, staged, drift-detected, and pruned as one set). This lowers OPM's adoption cliff for teams who already have manifests, without diluting the typed component happy path.

The feature lives entirely at the **apply layer**:

- `opmodel.dev/core@v0` and the library kernel are untouched. The kernel is pure by constitution and cannot read a filesystem or run Kustomize.
- Kustomize is rendered by the embedded `krusty` library, not a shelled-out binary.
