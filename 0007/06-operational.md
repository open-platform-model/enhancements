...
Five fixed prompts make up the OPM Production Readiness Review (PRR-lite), answered below.
...
- **Inventory.** Side objects appear in `status.inventory` like any rendered resource. No new inventory surface is added, but the inventory count/digest now includes them, which is the intended visibility.
