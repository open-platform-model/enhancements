...
`task update-deps` is the workaround, and it has three structural limits that no amount of polish fixes:

1. **It is pull-only.** A human must invoke it.
2. **It requires the full-workspace checkout.** It cannot live in any repo's CI.
3. **It produces no per-bump provenance.** The change is a bare version-string edit with no link to the upstream release.

These are properties of *where* and *how* the task runs, not bugs to fix. That is why a scheduled, per-repo, PR-generating mechanism is needed alongside it, rather than a better version of the task.
