...
4. **`releases`** — re-pin to the new coordinates.

The hard sequencing constraint sits outside this list: **step 3 must land in the same window as enhancement 0010's identity migration.** Both change what a module's identity resolves to, and running them separately moves every artifact's identity — and therefore every deployed resource's owner label — twice, with two adoption windows instead of one.
