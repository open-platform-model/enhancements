...
**The permanent constraint.** Enhancement 0010 D2 rejected a module version partly on a measured failure: because `fqn` interpolated `version`, a moving version changed `module.uuid` → `instance.uuid` → the owner label on every rendered resource, and `opm-operator/internal/apply/prune.go:107` skips any delete whose live label disagrees with `Status.InstanceUUID` — so every upgrade silently orphaned whatever it removed. That argument is defused **only** because 0010 D1 makes `fqn` the module path with no version in it. If a future change wires `version` back into `fqn` or `uuid`, the orphaning returns, and it reports success while doing it. This is an invariant of the identity design rather than a property of this decision, and it belongs in `core/SPEC.md` beside the field.
