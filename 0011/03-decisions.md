...
### D15: The authored version is the trigger; the registry is the idempotency check; nothing predicts a version
...
- **The idempotency check is a registry lookup.** Publish resolves whether that tag already exists. ~~If it does, there is nothing to do — the same client-side check D10 already specifies, used to *skip* rather than to *refuse* when publish is run as a fleet sweep, with the mode belonging to the command-surface gate.~~ **Reversed by D19: publishing an already-published version is ALWAYS a refusal.** There is no sweep mode and no skip. The lookup moves to the *sweep*, which resolves which artifacts carry an unpublished version and invokes publish only for those — so idempotency is a property of the caller rather than a tolerance inside the command. D10 had already specified the client-side refusal, which is what made the skip a carve-out from a rule this design had stated.
- **CI needs no trigger logic at all** — *narrowed by D19 rather than reversed*. A sweep over every module on `main` stays idempotent, but by filtering before it invokes rather than by publish no-opping: CI still needs no notion of whether an artifact **changed**, only of what the registry already holds. Running it twice, or on every commit, changes nothing.
...
**Rationale:** In the author's words — "I don't want to develop and maintain a complex system that tries to predict what version to publish, I would rather it be up to the authors." What makes this cheap rather than merely simple is that the trigger question dissolves once versions are authored: publishing an already-published version is a no-op, so a sweep needs no notion of "should this publish?" to be safe. The mechanism that was there to answer that question was answering it wrongly at a rate of ten modules in twelve.
...
**Revised:** 2026-08-04 — **amended by D19**, recorded the same day. The skip-in-sweep half of the idempotency bullet is **reversed**: an already-published version is always a refusal, and idempotency moves to the sweep's registry filter. The trigger half is untouched — the authored version is still what decides a release, nothing predicts a version, and no ledger is consulted — and the CI claim is narrowed rather than dropped. Both bullets above carry the correction inline so this decision cannot be read alone and come away with the superseded behaviour.
...
### D19: Publishing an already-published version is always a refusal; the sweep filters, publish does not tolerate

**Decision:** `opm module publish` and `opm catalog publish` **always refuse** when the version being published already exists in the registry. There is no sweep mode, no `--skip-if-published`, and no invocation in which the condition is a success. Publish checks the registry and errors, naming the artifact, the version, and that the version must be bumped.

**Idempotency moves to the sweep.** D15 makes a CI pass over every module safe by construction, and that property is unchanged — what changes is where it lives. The sweep resolves which artifacts carry an unpublished version and invokes publish only for those, rather than invoking publish on everything and relying on it to no-op. The filter is a registry lookup, so nothing is predicted and no ledger is consulted; D15's substance is untouched.

**This amends D15**, whose second bullet reads that the registry lookup is "used to *skip* rather than to *refuse* when publish is run as a fleet sweep" and whose third says "CI needs no trigger logic at all". The first is reversed. The second is narrowed rather than reversed: CI still needs no notion of *whether an artifact changed*, which is the trigger question D15 dissolved and the thing the checksum was doing wrong — it needs only to ask the registry what is already there.

**Alternatives considered:**

- **Skip in sweep mode, refuse when targeted**, as D15 wrote it and `04-graduation.md` carried as the last unsettled item on the command surface. Rejected by the author: it makes one condition mean two things depending on how the command was reached, and the mode itself would have needed a spelling nobody wanted to invent.
...
**Rationale:** In the author's words — "It should be a refusal. It should check against the registry and error." The consistency argument is the stronger one: **D10 already specifies this behaviour.** It has publish "refuse client-side to overwrite an existing tag", which is the same condition reached from the immutability side. A skip in sweep mode would have been a carve-out from a rule the design already stated, and carve-outs on refusals are how the producer-side half becomes decorative — the risk `05-risks.md` records under *a refusal that fires too easily gets routed around*, arriving from the opposite direction.

**Source:** User decision 2026-08-04. **Amends D15**; consistent with D10's client-side tag refusal. Closes the last item under `04-graduation.md`'s command-surface gate.
