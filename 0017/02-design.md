...
4. **Core honors trait optionality**: `#Component._allFields` projects an `optional: true` trait's spec through an optionalizing comprehension (`for k, v in trait.spec {(k)?: v}`), so the field exists as a constraint but is absent until set. It embeds an `optional: false` trait's spec as-is (required). Absence is what arms the transformers' existing per-kind fallbacks.
