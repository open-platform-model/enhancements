...
Four authoring layers can plausibly hold a default value for a component field, and today none of them can do the job:

1. **Trait/resource schemas** (`catalog_opm/src/traits/`) are workload-type-agnostic unions. `#UpdateStrategySchema.type` is `"RollingUpdate" | "Recreate" | "OnDelete"` with no `*` — correctly, since the right default (and the legal menu) depends on the target kind the trait cannot see.
...
- **Defaulting the trait schema** (`type: *"RollingUpdate" | …`): fixes only defaultable fields, at the wrong layer — a global claim the trait is not entitled to make (the menu and the default are kind-dependent), and it permanently spends the field's single default slot, so no downstream layer can ever default it differently.
