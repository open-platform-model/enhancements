...
- **`opm install` (D5).** Because inventory now lives in a CR, the `ModuleInstance` CRD is a hard prerequisite for every CLI apply. `opm install crds` (CRDs only) and `opm install operator` (full operator from `dist/install.yaml`) install via SSA with manager `opm-cli`; manifests are embedded at build time with a `--version` fetch fallback. `opm uninstall operator` never deletes CRDs (deleting a CRD cascades to every `ModuleInstance` in the cluster); CRD removal is a deliberate manual `kubectl delete crd`.
