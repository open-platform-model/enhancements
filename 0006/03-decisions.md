...
**Rationale:** Both actors read the same CR fields through the same kernel, so a faithful, self-contained `spec.values` is sufficient for byte-identical renders by construction: there is no interpretation gap to close. Forcing the CLI to render its *own* apply from `spec.values` (rather than from a richer effective set it then partially serializes) collapses the entire faithfulness question to "is `spec.values` the thing I rendered from?" The answer is yes by definition. The forward-guard records the exact condition under which a future feature would reopen the question, so the closure is honest today without building anything for a feature that may never land.
...
**Decision:** The CLI must be equal to or newer than the operator and CRDs running in the cluster; it **refuses the unsafe direction** (CLI older than the cluster) rather than maintaining a compatibility window. This is consistent with D14, since refusing the unsafe skew is cheaper than promising multi-version support. Enforcement is gate-only: two bounded checks, no per-field schema adaptation.
...
- Keep the full library package as originally scoped (D13/D26). Rejected: tracing which functions are actually compared or relied upon across the actor boundary shows only the entry shape qualifies.
  1. `ComputeStaleSet` is never compared cross-actor. D7.4's render-digest gate already guarantees `previous ≈ current` at the one moment a stale-set computation crosses actors (the operator's first post-handoff reconcile reading CLI-written entries), making the base-relation choice moot at exactly that instant.
  2. `ComputeDigest` has no cross-actor consumer at all. D2/D25 never has the CLI write a `lastAppliedInventoryDigest` the operator's `IsNoOp` fast-path reads, so the operator's first post-handoff reconcile always takes the real render+stale-set path regardless of digest-algorithm agreement.
  3. `ApplyComponentRenameSafetyCheck` is only meaningful paired with a specific stale-set base relation, not independently shareable.
  4. `CollidesOnApply`/`PreApplyExistenceCheck` decide only what *this* actor does before *its own* apply, never compared against the other actor's verdict for the same identity.
...
**Decision:** With the CLI on CUE ≥ v0.17 (D36), CUE's local module file (`cue.mod/local-module.cue`, holding `deps: "<path>@vN": replaceWith: "<local dir | fork@version>"`) is the sanctioned way to develop and apply against unpublished or locally-edited modules: instance packages keep their canonical registry imports, and the replacement lives in a file that is **never published and honored only in the main module**, so local-ness structurally cannot leak into a published artifact or into the CR.

Consequently the `ModuleInstance` CR never represents a filesystem path in any mode: (a) a module-dir-as-main-module apply (D6's local case) writes the module's **declared** `spec.module` path/version, always present since `#Module.metadata` requires both, so the CRD's `MinLength=1` constraints hold for every local apply; (b) a locally-replaced-dependency apply writes the canonical import verbatim and is indistinguishable in the CR from a registry apply. Handoff safety is unchanged and marker-free: D7.3 (resolvable reference) and D7.4 (digest re-render from the *published* reference) structurally abort on a diverged local checkout.

D6's `module-instance.opmodel.dev/source: local` annotation is **demoted from fixed behavior to OQ17** (generalized to "rendered bytes did not come from a registry-resolved artifact", covering both modes); if kept it is observability-only and MUST never be a gate input, since annotations are user-editable.

Operational notes: consuming `cue.mod/module.cue` files declare `language.version: "v0.17.0"` **exactly** (it is a consumer floor; declaring v0.17.1 would gratuitously lock out v0.17.0-line tools for zero gain), and C1's bump work verifies (i) the CLI loader resolves modules through the SDK's standard module resolution so `replaceWith` is honored rather than bypassed by the CLI's own OCI wiring, and (ii) a fully-replaced, never-published module path needs no registry existence (the docs' omit-`v`-for-replace-only-deps wording implies yes; one 5-minute check).
...
- **`cacheDir` is deleted too**: declared in the embedded schema and documented with an env var, referenced by zero Go code. Re-adding a cache knob later, if the kernel's OCI pulls warrant one, is additive.
- **D21's "local/embedded default" platform source is pinned to a sibling file `~/.opm/platform.cue`**: a data-only CUE file shaped as the Platform CR *spec projection* (name, type, subscriptions), not an import-bearing authored core `#Platform`. The CLI parses it (`CompileBytes` + embedded projection-schema validation, the same technique `config.cue` already uses), decodes it into `synth.PlatformInput`, and runs the same `SynthesizePlatform` → `Materialize` chain the operator's `PlatformReconciler` runs. All three D21 sources (`--platform <file>`, cluster `Platform` CR, local default) therefore converge on one ingestion path, which also strengthens D30's render-digest parity, since the CLI's platform ingestion is structurally the operator's own mapping rather than a separate file-driven load path. The CR-spec shape additionally makes D12's solo-cluster write-if-absent trivial (the un-owned `cluster` Platform is this file's contents wrapped in `apiVersion`/`kind`/`metadata`) and makes the file the graduation on-ramp: one `kubectl apply` from being the cluster Platform.
- **`~/.opm` stops being a CUE module.** With both files import-free, `~/.opm/cue.mod/` (and its pinned catalog dependency set) is deleted; `opm config init` stops running `cue mod tidy`; `DefaultModuleTemplate` goes. The two-phase config load: `BootstrapRegistry` regex-scraping `registry:` out of the file to set `CUE_REGISTRY` before evaluation: collapses to a single pass, since nothing in `~/.opm` ever resolves an import. Dead with it: `configHasProviders`, `extractProviders`, `GlobalConfig.Providers`, `GlobalConfig.CueContext` (the kernel owns its `*cue.Context`), `pkg/loader.LoadProvider`, `pkg/provider`, `resolver.resolveProvider`.
- **`opm config init` seeds `platform.cue` subscribing to the two official catalogs** (`opmodel.dev/catalogs/opm` and `opmodel.dev/catalogs/kubernetes`), with explicit, visible `filter.range` constraints rather than open ranges, so a fresh install never silently jumps a major (the catalog-FQN version-skew failure class). Both catalogs currently publish prerelease SemVers (`1.0.0-alpha.1`, `1.1.0-alpha`), so the seeded ranges must be prerelease-tolerant until the catalogs cut stable; the exact range expressions are fixed at C2 drafting against whatever the catalogs then publish.
- **`opm config vet` validates both files**; no separate `opm platform vet` command. A `platform` noun stays available for future verbs (e.g. a D12 push) without being required now.

Lands in **C2 (`cli-kernel-adoption`) as its first task group**, not a separate slice: today's render path consumes `GlobalConfig.Providers`, so deleting `providers` before kernel adoption leaves the CLI unable to render anything. There is no useful intermediate state.

**Alternatives considered:**

- A `config.platform` field inside `config.cue`. Rejected: a platform referencing core's `#Platform` drags CUE imports back into the config file, resurrecting the registry-bootstrap ordering and `cue.mod` dependency management this decision deletes.
- Embedded-only default (no `~/.opm` platform file; local always explicit via `--platform <file>`). Rejected: loses the zero-flag default D21 assumes, and loses the seeded file as the visible, editable record of which catalog versions the user renders against.
- Authored-`#Platform` shape for `platform.cue` (a `c.#Platform &` file). Rejected: requires imports and a `cue.mod`, and `#registry` is a hidden field awkward to author as data; authored platforms with imports remain first-class via `--platform <file>` pointing into a real CUE module. `~/.opm/platform.cue` is operational config, not authored schema.
- A separate pre-C2 config-cleanup slice. Rejected: only the inert parts (`cacheDir`) could land without breaking render; not worth a slice.

**Rationale:** The `providers` block was the sole reason `config.cue` had imports, and imports were the sole reason for `~/.opm`'s `cue.mod`, the `CUE_REGISTRY` bootstrap ordering, and the two-phase loader. The entire chain existed to feed a `#Provider` concept the 0001 rewrite removed from core. Deleting the root cause deletes the tree. Making the local platform a CR-spec-shaped data file keeps `~/.opm` import-free, unifies all three platform sources on the operator's own ingestion path, and converts hidden version selection (transformers resolved via `~/.opm/cue.mod` pins, the known `opm instance build` footgun) into a visible, declared subscription range in a file the user owns.

**Source:** User decision 2026-07-18 (explore-mode session: `~/.opm/config.cue` cleanup investigation; option A — sibling file — chosen over `config.platform` and embedded-only).

---

### D40: Handoff success is "inventory-stable reconcile", not byte-no-op (the managed-by relabel is expected and reported; no cross-actor digest comparison); amends D7.5

**Decision:** The kernel stamps the calling runtime's identity into every rendered resource (`core` `#TransformerContext.#runtimeName` → `controllerLabels."app.kubernetes.io/managed-by"`), so the operator's first post-handoff reconcile SSA-applies every resource with a one-label difference (`opm-cli` → `opm-controller`). D7's "zero changed" wait can never hold. C3 redefines the post-flip success criterion as an **inventory-stable reconcile**, observable from the CR's status alone: `Ready: True`, the `status.inventory` entry *set* identical to its pre-handoff value, revision incremented, zero pruned. The relabel is reported honestly (e.g. "operator adopted N resources: managed-by relabel, no workload changes"), not asserted away.

Two corollaries are binding on C3's implementation:

1. **No cross-actor digest comparison, ever.** After the flip the operator writes its *own* `lastApplied*` digests, which differ from the CLI's by the same label by construction. The only valid digest check in the flow is D7.4's pre-flip self-comparison (CLI re-render with its own runtime name vs. its own recorded digest).
2. **The blast radius is confirmed label-only.** Catalog workload transformers build `spec.selector.matchLabels` and pod-template labels from runtime-independent `componentLabels`, so the relabel is a metadata-only patch: no selector-immutability violation, no rolling restarts, no workload disruption.

The zero-*downtime* property survives; the zero-*diff* property was never real and is dropped. Drafting note for C3: verify the operator's prune-time ownership guard accepts resources still labeled `opm-cli` in the window where a resource is removed from the module before any relabeling reconcile ran.

**Alternatives considered:**

- Strict per-resource verification (GET every resource post-reconcile, assert only `managed-by` changed). Rejected: expensive, racy against concurrent legitimate changes, needs read RBAC on every kind, and duplicates what D7.4 plus the structurally-empty stale set already guarantee.
- Neutralize the label (constant `opm`, or move runtime identity to an annotation) to restore a byte-neutral handoff. Rejected: a core + catalog + both-runtimes change that destroys a genuinely useful operational signal (`kubectl` shows who manages the resource *now*) to rescue a cosmetic property nothing depends on; it would also retroactively invalidate C2's shipped parity framing (parity holds with runtime name held constant, `tests/integration/render-parity`, byte-identical).
- Digest normalization (strip `managed-by` before digest computation on both sides). Rejected: restores cross-actor digest equality that no requirement consumes: D7.4 is a self-comparison and the operator's post-flip digests are informational.

**Rationale:** The property the user actually needs from handoff is "the operator now owns exactly what I deployed, and nothing was destroyed, recreated, or restarted," which the inventory entry set (identity tuples, label-free) plus the empty stale set plus Ready expresses precisely, from status reads alone. "Zero changed" was a proxy for that property that happened to be false in a system where render output encodes the renderer. Reporting the relabel keeps the CLI honest about what the reconcile did without failing a successful adoption.

**Source:** User decision 2026-07-18 (explore-mode session on the C2 relabel discovery; option A — inventory-stable reconcile — chosen over per-resource verification and label neutralization). Amends D7's step-5 no-op expectation; D7.1–7.4 stand. Grounded in `core/src/transformer.cue` (`controllerLabels`), `catalog_opm` workload transformers (`componentLabels` in selectors/templates), and C2's parity result (archive `cli/2026-07-18-cli-kernel-adoption`, task 4.2).
