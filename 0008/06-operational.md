...
Five fixed prompts make up the OPM Production Readiness Review (PRR-lite):
answer every one, even briefly. The answers tell future operators,
contributors, and on-call responders what to expect when the enhancement
lands. Leave a prompt blank only if it is genuinely N/A; say so explicitly
when it is.
