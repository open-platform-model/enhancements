...
- **OQ4: Does the `ModulePackage` kind have a `core` domain definition to anchor its `#CRD`, or is it operator-only?** Status: open. `ModuleInstance` and `Platform` map cleanly to existing `core` definitions; `ModulePackage` (Flux-sourced) may be operator-specific. If operator-only, either add a minimal `core` definition or scope `ModulePackage` out of the first slice. Resolved by checking `core/src` for a ModulePackage shape and deciding ownership.
- **OQ5: What is the semver impact on `opmodel.dev/core`?** Status: partial. Adding `#CRD` and `#CRD` instances is additive (minor) for `core`'s schema. But if anchoring the CRDs forces any change to the existing `#ModuleInstance`/`#Platform` field shapes, that could be breaking. Resolved once the `core` slice is spiked and the diff to existing definitions is known (feeds `config.yaml.semver`, required for `accepted`).
