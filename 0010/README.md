...
- Where identity lives and how it gets there: a committed `identity.cue` with `@opm()` markers, in the module's own root package or in a catalog's `identity/` subpackage, with fields that may be open or concrete.
...
- **Module version selection** — how a consumer pins or ranges a *module* dependency. This entry fixes what a version means; choosing one is separate. Catalog subscription selection is the exception and is **in** scope (D14, D15), because under D13 it is the mechanism that produces cross-minor compatibility rather than a policy layered on top of it.
...
- **The catalog repackage** — composition and materialization semantics. Enhancement 0001 owns those. The boundary moved with D13 and is worth stating precisely: `#SubscriptionFilter`'s *default* and its prerelease opt-in are in scope here, because SemVer keys make them load-bearing for whether a module matches at all; how catalogs are assembled, filtered by kind, and composed remains 0001's.
...
| `core/src/resource.cue`, `core/src/trait.cue`, `core/src/blueprint.cue`, `core/src/transformer.cue` | Primitive `fqn` derivations — the SemVer tail is kept (D13); the path portion changes with `modulePath` |
...
| `library/opm/materialize/filter.go` | `filterVersions` + `highestStable` — the empty-filter default D14 replaces and the prerelease rule D15 replaces |
