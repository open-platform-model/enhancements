...
| `core/src/module.cue` | `#Module.metadata` — `version` **retained and in no key** (D38), `modulePath` reshaped, `fqn` redefined, `registryPath` added (D41), version-major agreement added (D40), the version label **retained**, schema-declared and kernel-verified (D9) |
