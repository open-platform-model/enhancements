...
- **OQ2: What does the D3 floor compare against when a catalog was not resolved from a tag?** D4 replaced the floor entirely: a contract key now names an API version rather than a build, so there is no build-comparison left to resolve. Status: resolved-by-D4.
...
- **OQ5: Do prereleases and 0.x builds of one major share a key space safely?** Yes: D4's contract keys carry no build at all, and D14's subscription names one build explicitly, so a prerelease is selected by being written down rather than by ranking. Status: resolved-by-D4 and D14.
...
- **OQ9: What supplies the auto-secrets resource identity, now that `core` cannot?** Status: answered. Nothing does: `core` no longer names one.

  `core/src/helpers_autosecrets.cue` was deleted and the `opm-secrets` injection removed from `#ModuleInstance` (core `a77a12d`, 2026-07-27), so no `core` construct names a catalog primitive and there is no identity left to supply.
  ...
  Original framing follows. Surfaced 2026-07-27 while resolving OQ3. `core/src/helpers_autosecrets.cue:5` hardcodes `#SecretsResourceFQN: "opmodel.dev/opm/resources/config/secrets@1.0.0"` and `:31` a matching `modulePath: "opmodel.dev/opm/resources/config"`. These are **live values, not doc examples**: `core/src/module_instance.cue:72` synthesizes an `opm-secrets` component keyed by that FQN whenever a module's resolved config contains `#Secret` fields. The shipped catalog ships the same primitive at `opmodel.dev/catalogs/opm/resources/secrets@<id.Version>` (`catalog_opm/src/resources/secret.cue:20-22`), and `catalog_opm/src/transformers/secret_transformer.cue:35` requires *that* FQN. The two differ in both path and version, so the synthesized component cannot match the transformer meant to handle it. The stale comment above the constant ("must stay in sync with `resources/config/secret.cue`") names the deprecated `catalog/` tree's layout, so this is fallout from the catalog split. One fleet module reaches the code path (`modules/metallb`).

  This belongs to 0010 because D13 makes the version half permanent: a literal `@1.0.0` written into `core` can never equal a catalog build's key, and no subscription breadth fixes it. The key names a build of a catalog that does not exist. The deeper problem is that `core` names a *catalog primitive* at all, which is a layering inversion this entry's identity model has no way to express: `core` is the schema every catalog depends on, so it cannot depend on one.

  Candidates:

  1. Move the whole auto-secrets synthesis out of `core` and into the catalog that owns the secrets resource, so the FQN is computed from `id` at its own definition site like every other primitive.
  2. Keep the synthesis in `core`, but have it take the secrets FQN as an input supplied by the platform's materialized catalogs rather than as a literal.
  3. Keep the literal and accept that auto-secrets only works for a catalog that agrees to ship that exact path and version, documenting the coupling.

  Resolving this fixes whether `core` may name a primitive, and whether auto-secrets survives in its current form.

- **OQ10: What owns a `ModuleInstance`'s resources through deletion?** Status: deferred-to-0012.

- **OQ11: What pins the transformer build, now that the key no longer does?** The subscription does: it names its build directly, so resolution is a projection of committed source rather than something a key has to pin. Status: resolved-by-D14.
...
- **OQ15: Does a subscription still materialize every build in the major, and if so what picks one transformer per contract?** No. A subscription names exactly one build, so two builds of one catalog can never reach the same contract bucket and there is nothing left to pick between. Status: resolved-by-D14.
