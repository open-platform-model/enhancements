...
- As a **platform team operator**, I want to upgrade a catalog by a minor version without breaking the modules installed on my platform, so that catalog releases are routine. Today: my subscription materializes one build, so the moment I take a newer one every module built against the old one fails with `no matching transformer` — and I must rebuild the whole fleet in lockstep. I also want the reverse guarantee, which nothing offers today: that publishing a catalog build does not change what an already-installed module renders.
