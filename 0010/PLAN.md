...
| docs-catalog-contract | implementation | opmodel.dev | planned | core-alpha-release | Publish the catalog-author contract (additive-only rule keyed to level, verified only via `opm catalog publish`, three version-shaped values) plus the registry-namespaces reference: owned prefixes, enforcement points, decisions.   |
