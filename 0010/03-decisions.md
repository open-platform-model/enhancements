...
D13's reproducibility argument survives this reversal and is not being discarded — it is being **relocated**, which the entry must say plainly. Under D13 the key did the pinning: a module's authorship pin selected the transformer build permanently. Under D24 the key pins nothing, so the platform's subscription must, and today it floats (`filter.go:43-47` selects the highest published stable, and `MaterializedPlatform.Resolved` at `materialize.go:109` is in-memory only, never recorded or compared). Correctness no longer depends on it — under D27 any build in the major is a legal supplier — but byte-stable renders do. That is OQ11, and it is the price of admission for this decision rather than a follow-up.
...
The boundary is not "all of `metadata`", and OQ12 exists because of it. `catalogVersion` and `description` are provenance. `name`, `modulePath`, `apiVersion` and `fqn` are identity that the map key already forces into agreement, so excluding them is harmless but not meaningful. `labels` are neither: component labels are the union of the labels of its attached primitives, and transformers match on `requiredLabels`, so a label added in a later build changes the component's label set and is a **contract** change rather than provenance.
