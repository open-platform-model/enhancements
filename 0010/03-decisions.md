...
**Superseded by D29** — there is no filterless subscription any more. `versions` is required, so a subscription that names nothing fails to unify instead of falling through to a default. D14's premise stands as history: under D13 a platform had to cover its fleet's authorship history, and D24 is what removed that requirement.
...
**Superseded by D29** — the flag is deleted rather than implemented. It existed to reconcile "the default selects every build in the major" (D14) with "prereleases need opt-in"; D29 deletes the default, so a prerelease is selected by appearing in `versions` and needs no separate signal. The measurement below stands and is why D29 could not leave prereleases to constraint syntax either.
...
### D24: A primitive's key is its own API version; a transformer's key keeps the build SemVer

**Decision:** `#Resource`, `#Trait` and `#Blueprint` FQNs become `path/name@vN`, where `vN` is a **per-primitive API version** — that primitive's contract major, moved when its shape breaks, independent of the catalog's module major and of the catalog's release SemVer. `#ComponentTransformer` FQNs keep the full SemVer of the build they shipped in. There are therefore two FQN types, split by role rather than by taste.
...
**This supersedes D13's keying half**, and with it D14's role as the compatibility mechanism. What D13 got right survives and is restated below.

**The failure that forced it, and it is not hypothetical.** Walked 2026-07-29: `catalog_opm` defines a generic `backup` resource and trait with **no transformer of its own**, because the contract is meant to be fulfilled by whatever provider a platform installs; a `k8up` provider catalog ships the transformer that requires it. Under D13 the demand key is `…/opm/resources/backup@1.1.0` — the exact `catalog_opm` build the *module* compiled against — and the supply key is whichever `catalog_opm` build the *provider* compiled against, fixed by the provider's own `cue.mod` because `materialize/pull.go:23` loads each catalog as its own root. They must be **equal**, not compatible. D14's subscription breadth cannot repair it: subscribing to every `catalogs/opm` build supplies every build's own transformers, and `backup` has none, so `catalogs/opm` contributes nothing to that bucket at any version. Coverage is instead the set of `catalog_opm` versions the provider's release history happens to have pinned — one per provider build. Every `catalog_opm` release therefore breaks backup for modules that adopt it until the provider re-releases and the module is rebuilt to match: an N×M lockstep between two independently-released catalogs, which is the coupling D15's own rationale names as what this entry exists to remove.

**Alternatives considered:**

- **Keep D13's SemVer keys everywhere.** The status quo. Rejected on the coverage argument above: cross-catalog fulfilment works only by coincidence of release timing, and the remedy the diagnostic implies — "rebuild your module against whichever catalog build your platform's provider was compiled against" — points at the wrong party and is not an action a module author should ever be asked to take.
- **D4's catalog-major keys** (`@v1` meaning the *catalog's* major). Rejected, and this is the distinction that matters: one version namespace for every primitive in a catalog means breaking one trait's shape requires a catalog major bump — a new module path, a new import in every consumer, a whole-catalog migration — so in practice it never happens and "v1 means v1" becomes unenforceable by construction. A per-primitive API version gives a catalog a legitimate, cheap way to break one contract on purpose, which is the only thing that makes the promise in D27 keepable.
- **Major keys for transformers too.** Rejected on the mechanism that killed D4: two builds of one catalog collide on one composed-map key and `materialize/index.go:64-70` raises `transformer %q diverges across selected builds` for any transformer whose body changed. Keeping the build in the transformer key keeps `index.go`'s stated invariant true and leaves multi-build subscription structurally available (its fate is OQ15).
- **A range-aware matcher over SemVer keys.** Rejected for the reason D4 and D13 both rejected it: it turns an O(1) keyed lookup into constraint solving, and it does not address the cross-catalog case at all, because the provider's key is not a range either.
...
D13's reproducibility argument survives this reversal and is not being discarded — it is being **relocated**, which the entry must say plainly. Under D13 the key did the pinning: a module's authorship pin selected the transformer build permanently. Under D24 the key pins nothing, so the platform's subscription must. Correctness mostly survives without it, but not unconditionally, and the asymmetry matters: under D27 a supplier build is a safe substitute when its contract surface is a **superset** of the one the module compiled against — same build or newer — while an **older** supplier is safe only while the module stays inside the fields that build already had, and fails loudly (`spec.backup.retention: field not allowed`) the moment it does not. Byte-stable renders get no such protection in either direction. That was OQ11, resolved by **D29**: the subscription names its builds, so the pin is back in committed source and the float is gone.

It also buys a capability D13 could not express: two API versions of one primitive shipping **side by side** in a single catalog build, the way Kubernetes serves `v1beta1` and `v1`. Providers implement the new one when ready, modules migrate when ready, both keys sit in the index at once, and the old contract retires when nothing demands it. Under SemVer keys every release breaks every key, so a deliberate break is indistinguishable from a patch.

**Source:** User decision 2026-07-29, on a scenario walk of a provider-fulfilled backup contract. Cross-catalog coverage read from `materialize/pull.go`, `materialize/filter.go` and `compile/match.go` 2026-07-29. Match-time mechanics measured in `experiments/02-primitive-closedness-skew/`.

### D25: `apiVersion` is authored per primitive; `version` is renamed `catalogVersion`

**Decision:** `#Resource`, `#Trait`, `#Blueprint` and `#ComponentTransformer` gain a required `metadata.apiVersion` (`#APIVersionType`), authored per primitive, which is what a contract FQN interpolates under D24. `metadata.version` is **renamed** `metadata.catalogVersion`, keeps its full SemVer and its source (`id.Version` / the `#Catalog` pattern stamp), and becomes provenance: it records the build a definition shipped in and is never part of a contract key.
...
- **Keep the current soft treatment.** Rejected: it was survivable while every demanded primitive came from a catalog that also shipped its transformer. D24 makes an unsupplied demand a normal condition, and a silent one is indistinguishable from a fulfilled one.
...
### D29: A subscription names its catalog builds explicitly — `range`, `deny`, and the empty-filter default are deleted

**Decision:** `#SubscriptionFilter` (`core/src/platform.cue:16-20`) collapses to one field. `allow` is renamed **`versions`** and becomes **required and non-empty**; `range` and `deny` are deleted; the empty-filter default is deleted with them. A `#Subscription` names the exact bare SemVers it materializes, and one that names none is a schema error rather than a resolution that quietly picks something.

Catalog selection therefore becomes a **pure function of committed source**: no highest-stable rule, no SemVer constraint solving, no registry ordering, and no maturity inferred from constraint syntax. `git`-identical inputs materialize identical catalog bytes on any day, from any machine, with no lockfile — because the platform file *is* the resolution.

**This supersedes D14 and D15 in full.** D14's whole-major default existed because D13's build-keyed contracts forced a platform to cover the *authorship history* of its installed fleet; D24 removed that requirement, so breadth is no longer implied. It stays *expressible* — `versions` is a list — and what happens when two listed builds ship the same transformer is OQ15, unchanged. D15's prerelease opt-in goes with the mechanism it existed to reconcile: a literal `1.0.0-alpha.2` in `versions` is selected by being written down, so there is nothing left to opt into.

**Alternatives considered:**

- **Keep ranges and add a lockfile** (OQ11 candidate (b)). The more expressive answer and the one to reach for if manual bumps become the bottleneck. Not chosen now because it is strictly more machinery for the same guarantee, and because it stays available: reintroducing `range` alongside a recorded resolution takes nothing away from a platform that already names its versions. The reverse is not true — an established floating default cannot be withdrawn cheaply once platforms depend on it.
...
- **Keep `deny` as a subtractive tool.** Rejected: `deny` exists to carve exceptions out of a range. With no range there is nothing to carve — removing an entry from `versions` is the entire operation, and it is visible in a diff.
- **Delete `range` but keep the empty-filter default.** Rejected: the default *is* the float. `highestStable(published)` moves on the next catalog release whether or not a constraint was ever written, which is the exact mechanism OQ11 describes.

**Rationale:** Ranges without a lockfile is the one combination that cannot be made reproducible, and every ecosystem shipping ranges ships a lock beside them. OPM ships ranges and records its resolution in `MaterializedPlatform.Resolved` — an in-memory map whose only non-test consumer in the entire workspace is an integration harness (`cli/tests/integration/platform-materialize/main.go:95`), and whose own doc comment instructs callers that they "MUST NOT branch behavior on it". There were two exits from that state, build the lock or drop the ranges; dropping is smaller, closes more questions, and is the one that can be walked back.

It is subtraction, not addition. `allow` already exists and `filterVersions` already implements it, so the shipped change is a rename, a required marker, and three deletions — most of `filter.go`'s 145 lines and the Masterminds constraint dependency go with them.
...
**Source:** User decision 2026-07-29. `allow`'s existing shape read from `core/src/platform.cue:16-20` and `library/opm/materialize/filter.go` 2026-07-29; `Resolved`'s consumer set verified across `library`, `cli` and `opm-operator` the same day. The prior occurrence of the floating-resolution failure is enhancement 0006 OQ18, analysed in `0006/research/dev-tag-range-pollution.md`.
...
### D31: A subscription names exactly one catalog build — `versions` becomes `version`

**Decision:** D29's list collapses to a scalar. `#SubscriptionFilter` is **deleted outright** rather than reduced further — with one field left there is no filter to wrap — and `#Subscription` carries a required **`version`**: one bare SemVer naming the single build that subscription materializes. A platform that wants two builds of one catalog cannot express it. It makes two platforms.

The field is spelled **`version`, singular**, not kept as a one-element `versions` list. That choice has a price and it is recorded here rather than discovered later: widening back to multi-build becomes a breaking rename instead of a list relaxation. The name describes what the shape means today rather than holding a door open for a capability with no established use.

**This supersedes D29's list shape and dissolves OQ15.** D29 kept `versions` a list on the reasoning that "deliberate breadth is still legitimate", and deferred to OQ15 the question of what happens when two named builds ship one transformer. Examined directly, the legitimacy does not survive:

- **Union coverage** — build `1.0.0` ships transformers A and B, build `1.2.0` ships A only, and listing both yields `A@1.2.0` + `B@1.0.0`. This is the one case multi-build uniquely serves, and it describes a catalog that made a breaking change without saying so. Under D28 the dropped transformer now fails the render loudly; the fix belongs to the catalog author bumping a major or restoring B. A platform quietly compensating with a second subscription is precisely the implicit repair D24 and D29 have been removing.
- **Gradual migration** — does not structurally exist. Under D24 a module demands resources and traits and never demands a transformer, so no module can "stay on the old build". The platform picks and every module rendered against it gets the same transformer. There is no per-module transition for breadth to stage.
- **Two `apiVersion`s of one contract** — a single build ships `backup@v1` and `backup@v2` side by side. That is what D24's contract keys are for; no second build is involved.
- **Testing a new build beside the old** — two platforms, which is already expressible, names both behaviours, and costs nothing hidden: Materialize is explicit and caller-driven and the kernel holds no cache (0001 D14).

**Alternatives considered:**

- **Keep `versions` as a one-element list** (`[#VersionType]`), so widening later is a pure relaxation and CUE list arity rejects a second entry loudly. Considered and declined by the author. It buys a cheaper reversal at the cost of a shape that describes an intention rather than a rule, and it invites the reader to try two entries and discover the constraint by failure. The reversal cost is real and is stated above rather than hidden.
- **Keep breadth with a newest-wins tie-break** (OQ15 candidate (b)). Defensible on its own terms — D27's additive-only promise makes the newer build's contract a superset, so the newer transformer cannot lose capability, and the winner is a max over a committed list so D29's inertness survives. Rejected because it makes listing two builds *indistinguishable in effect* from listing one in every case except the dropped-transformer case above, which is a catalog bug. It buys a silent arbitration to serve a scenario that should fail loudly.
- **Keep breadth and refuse overlap at materialize** (OQ15 candidate (c)). Rejected as the *same-catalog* rule: it makes the list legal to write and illegal to use for every catalog that did not drop a transformer, which is a shape that exists only to be rejected. The candidate survives in D32, where the operands are two different catalogs and the refusal is the whole point.
- **Keep `range` and add a lockfile.** Rejected at D29 and not reopened; D31 moves further in the same direction and the argument is unchanged.

**Rationale:** D29 removed resolution and kept breadth because breadth cost nothing to keep. It turned out to cost OQ15 — the last question gating this entry — and the capability it preserved has no use case that survives contact with D24 and D28. Deleting it dissolves the question rather than answering it, which is the same move D29 made on OQ7.

The subtraction runs deeper than D29's. `filterVersions` does not become a validation pass, it disappears: what remains is a single major-agreement check on one string, which belongs beside the subscription rather than in a file of its own. `#SubscriptionSelection._majorAgrees` collapses from a comprehension to one comparison. And `index.go:57-70`'s collapse-on-shared-FQN branch becomes unreachable for two builds of one catalog, because there are no longer two; it stays as written because two *different* catalogs stamping one `modulePath` is prevented by a publish gate (D17) and not by the schema, which is exactly the defensive case its comment already describes.

One consequence outside this entry's scope, recorded because it narrows an open question rather than because it needs deciding here: OQ13 candidate (b) — "materialize compares a newly-pulled build against another build of the same catalog it already holds" — is no longer available. Materialize never holds two builds of one catalog. OQ13's answer space is now publish-side-only or a standalone check.

**Source:** User decision 2026-07-30, including the singular spelling over the one-element-list alternative. The multi-build scenario walk is recorded above; `filterVersions` and `#SubscriptionFilter`'s current shape read from `library/opm/materialize/filter.go` and `core/src/platform.cue:16-34` on 2026-07-29 (D29) and unchanged since.

---

### D32: Two catalogs supplying one contract is a materialize-time error

**Decision:** After D31 a `#matchers` bucket holding more than one transformer means exactly one thing — two *different* catalogs supply the same contract — and that is a **hard error at materialize**, naming both catalog paths and the contract key. It is raised in the kernel, so the CLI and the operator inherit one implementation. A CLI platform validation command that walks a platform's catalogs and reports the same condition before a deploy is a convenience on top, not the guard.
