...
- **OQ13: Does the additive-only promise get a read-side check, or is publish-side enforcement accepted as the whole of it?** Status: open. D27 puts enforcement at publish (0011 D9) and at match. The publish gate is transitive — subsumption chains, so checking against the immediate predecessor secures the whole history — but only while every build goes through it, and D11 records that `cue mod publish` keeps working and is what every artifact published to date used. So a catalog published outside `opm catalog publish` breaks the chain with nothing reporting it, and the match rung catches only violations a module happens to exercise. This is the same argument D11 answered for identity by moving verification to the read paths; the question is whether compatibility deserves the same treatment or whether the cost is disproportionate. Candidates: **(a)** publish-only, documented as such, with the contract stated for third-party catalog authors; **(b)** materialize compares a newly-pulled build against another build of the same catalog it already holds, which is free of registry cost but only fires when more than one build is materialized (so it interacts with OQ15); **(c)** a standalone `opm catalog registry check --compat` extending 0011 D7, run deliberately rather than on every render. Resolving this fixes how strong the promise actually is for a catalog OPM does not publish.
