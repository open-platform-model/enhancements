...
**Source:** User decision 2026-07-26.
**Revised:** 2026-08-03 — **amended by D39**, which returns the label to `core` sourced from D38's `metadata.version` and turns the kernel into its verifier. The rationale above held while a module declared no version, or declared one that drifted; it does not survive a render path with no coordinate to stamp. `cli/internal/workflow/render/module.go:99` records that a module apply *always* renders a local directory, so the actor this decision relies on does not exist there. What carries forward is the requirement that the two frontends cannot disagree — met more strongly by both reading one committed value than by both writing the same computed one.
...
**Revised:** 2026-08-03 — **amended by D44**: the `apiVersion` clause binds the three **primitives** only. `#ComponentTransformer` is an adapter rather than a primitive (`core/SPEC.md:38`), and D34 stated the correct scope two days after this decision without amending it, so the log asserted both readings and `schemas/target.cue` followed this one. The **`catalogVersion` rename is unaffected and still applies to all four kinds** — a transformer's `catalogVersion` is its own key's source component (D4) and the provenance both ends of a match read (D26).
...
**Revised:** 2026-08-03 — the invariant this decision states is **restated by D41** and is no longer the whole of what protects `prune.go`. D38 wrote it as "`version` is never an input to `fqn` or to `uuid`", which reads as a prohibition and is, taken literally, already violated: the *major* reaches both, through the module path. D41 splits the rule in two — artifact identity distinguishes majors and nothing finer; instance identity, which is what the owner label carries, derives from the module's major-free `registryPath` and is reached by neither the version nor the major. The paragraph above is superseded by that pair; what survives unchanged is that **minor and patch reach neither `fqn` nor `uuid`**, and that a change wiring them in restores a silent orphaning. The read-side comparison this decision left open is **resolved by D39**, which adopts it.

---

### D39: The version label returns to the schema, and the kernel verifies rather than supplies it

**Decision:** `#Module.metadata.labels` declares `"module.opmodel.dev/version": "\(version)"` again, sourced from D38's `metadata.version`. The kernel stops being the label's *source* and becomes its *verifier*: at each read point D11 names, an acquired artifact's `metadata.version` is compared against the tag it was fetched by, and a disagreement raises the same typed error D11 raises for an address mismatch.

This **amends D9**, and it **resolves the comparison D38 deliberately left open** — the label is what makes the declared version load-bearing at runtime, and the acquire-time check is what makes it honest.

**Alternatives considered:**

- **Keep D9 as written — the kernel stamps the label from the resolved coordinate.** Rejected on a render path D9 did not account for. `cli/internal/workflow/render/module.go:99` records that "a module apply always renders a local module directory (the main module is local), so render provenance is local (enhancement 0006 D7)" — so the CLI's primary module path has **no resolved coordinate to stamp**. Under D9 alone that path either omits the label or invents one, while the operator's registry-acquired render stamps it. `04-graduation.md`'s byte-identical-render gate does not catch this: it compares `cli` against `opm-operator` for one *instance*, and both sides of that comparison acquire from a registry.
- **Drop the label.** Rejected at D9 and still rejected for D9's reason: selecting deployed resources by module version is a real operational need, and removing a field is not a reason to remove a capability.
...
- **Have the kernel overwrite the schema-supplied value wherever a coordinate exists.** Rejected on mechanics: CUE cannot unify a concrete label value to a different one, so this needs either a disjunction default in `core` or a post-render Go rewrite. Both reintroduce the very divergence between the two render paths that this decision removes, and the Go form puts a label's value outside the rendered CUE where the digest gates cannot see how it got there.

**Rationale:** D9's argument was that the label "describes an *acquisition*, not a definition", and that the code which fetched an artifact is the only actor that knows which one it got. That was correct while `metadata.version` drifted from its tag. D38 removes the drift at the producer — 0011 D12's publish check asserts `metadata.version == id.Version`, and the tag derives from the same value — and this decision removes it at the consumer. Declared and resolved become the same fact, and only the declared one is available on **both** render paths.

The division of labour is the point. The schema states what the artifact says it is; the reader refuses an artifact where that is not what it was fetched as. That is a stronger guarantee than stamping the coordinate, because a stamp makes the label true about *this render* while leaving the artifact's own claim about itself unexamined — and it is the artifact's claim that every other consumer reads.

**Source:** User decision 2026-08-03. Local-render path read at `cli/internal/workflow/render/module.go:99` the same day; the label chain confirmed at `core/src/module.cue:36`, `core/src/module_instance.cue:28` and `core/src/transformer.cue:131-137`; drift measurement from `01-problem.md`. **Amends D9**; resolves the read-side comparison D38 left open.
