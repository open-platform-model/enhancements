...
Design Goals and Non-Goals together define the boundary of the enhancement; the High-Level Approach should be understandable without deep implementation knowledge. All trade-off reasoning lives in `03-decisions.md`, not here.
...
1. **`metadata.modulePath` is the artifact's complete CUE module path, major suffix included**: `opmodel.dev/modules/postgres@v2` or `community.opmodel.dev/m/acme/postgres@v2`, not a bare prefix that has to be recombined with a name. `metadata.fqn` is that same string, and `uuid: SHA1(OPMNamespace, fqn)` keeps its formula with a version-free, major-bearing input. The declared path *is* the registry address, so nothing is recombined from parts and the address is recoverable by reading one field. (The namespace itself is 0011's: D13 there, which keeps first-party modules at `opmodel.dev/modules/<name>`.)

2. **`#Module` declares a version, and it is never part of identity.** A module states its version in an identity subpackage alongside its path, and the root package derives `metadata.version` from it (D2). What the version is *not* is an input to `fqn` or `uuid`: the major, which is the one version component both CUE and Go treat as identity-bearing, arrives inside the module path, and the full version stays out of every key. The two values cannot drift, because publish asserts the declared version against the tag it publishes at (0011 D12). That is what makes keeping the field safe now that `fqn` no longer interpolates it.
...
7. **A contract declares where its fulfilment comes from, and a provider-fulfilled contract admits exactly one provider** (D37, correcting D32's mechanism). `#Resource` and `#Trait` carry `fulfilment: *"catalog" | "provider"`. `"catalog"` is the default and today's behaviour: the declaring catalog implements it, and many transformers may consume one contract to produce different outputs. `"provider"` says fulfilment comes from another catalog: the shape D4 exists to enable, a generic `backup` declared in `catalog_opm` with no transformer of its own and fulfilled by a k8up provider catalog.

   For such a contract the platform must carry **exactly one** transformer requiring it: two is refused at materialize naming both catalog paths, zero is already D28's unresolved demand. k8up or Velero, never both; switching providers replaces a subscription rather than adding one.

   The declaration cannot be derived: D17 records that a primitive's owning catalog is not readable from its FQN. So the contract author states it. `#Blueprint` is excluded because a transformer can never demand one. **D32's bucket-arity test does not survive this:** `materialize/index.go:82-95` indexes required ∪ optional demands, so `catalog_opm`'s own `#ContainerResource` bucket holds 8 transformers from one catalog and `#VolumesResource` 6. The guard counts **required** demands on a `"provider"` contract; `compile/match.go`'s candidate loop stays unchanged, as D32 intended.
...
| What the version is for | n/a | the `catalogVersion` every primitive stamps as provenance (D25); no key interpolates it |
...
- **`#ModuleIdentity`** / **`#CatalogIdentity`**: the metadata shapes after the change, with `fqn` bound to `modulePath`, the module-side leaf constraint expressed over one field, and a `registryPath` projection (D41). Neither carries a version-major agreement: D40 put one on both, D43 removed it from `#Catalog` and D45 from `#Module`, leaving `#IdentityPackage` as the sole assertion.
...
- **`#CatalogMemberFQNGate`**: what 0011's publish gate asserts for one member of a catalog, primitive **or** transformer. Its four-kind scope is correct and survives D44; only its name changed, from `#PrimitiveFQNGate`. Kept deliberately outside `#PrimitiveIdentity` because expressing it there would re-derive the value and undo D21. This is where the FQN-versus-identity agreement is enforced: a contract key must read `kindPrefix[kind]/name@apiVersion`, a transformer key `…@catalogVersion`, and `catalogVersion` must equal `identity.Version` in both cases (D25). `declaredAPIVersion` is optional and conditionally required for the three primitive kinds, since a transformer declares none (D44).
- **`#FetchedArtifact`**: the read-side invariant, now in **two** clauses. An artifact lives where its metadata says it lives (D11), *and* it is the version it says it is (D9). The second is what D2 made possible and left open; D9 adopts it, because D9 keeps the `module.opmodel.dev/version` label in `core` sourced from the declared value, and a declared value that reaches a rendered resource has to be one a reader refused when it disagreed with the tag.
...
- `core/src/module.cue:22`: `version!` **retained** (D2); the module's identity subpackage supplies it and neither `fqn` nor `uuid` reads it. **D45:** `core` asserts no agreement between its major and `modulePath`'s: `identity/identity.cue`'s `VersionMajor: Major` is the only assertion, exactly as for `#Catalog` under D43. D40 had put a second copy here; D45 supersedes that half.
- `core/src/module.cue:23`: `fqn: modulePath`. A `registryPath` projection is added beside it (**D41**), exposing `#ArtifactRef`'s already-computed `registryPath`; it is what `#ModuleInstance` derives from, and it is the OCI repository every address-composition site in `cli` and `library` collapses into.
...
- `core/src/module.cue:36`: the `module.opmodel.dev/version` label **stays in the schema** (**D9**), sourced from D2's `metadata.version`. A kernel stamp sourced from the resolved coordinate cannot serve both frontends: `cli/internal/workflow/render/module.go:99` records that a module apply *always* renders a local directory, so on that path there is no coordinate to stamp. The kernel's work is on the read side instead: see `library/opm/helper/loader/registry/module.go` below.
- `core/src/module_instance.cue:25`: **D41.** `metadata` gains an explicit `fqn: "\(#moduleMetadata.registryPath):\(name):\(namespace)"`, and `uuid` derives from that rather than interpolating `#moduleMetadata.uuid` inline. This is the one edit in this entry that touches a shape enhancement 0001 owns; it is here because the value it fixes is the owner label, which is this entry's migration surface.
- `core/src/catalog.cue:10`: `#CatalogFQNType` retired or redefined.
- `core/src/catalog.cue:63`: `version!` kept; the `*"0.0.0-dev"` default removed. **D43:** no version-major agreement is asserted here: `identity/identity.cue`'s `VersionMajor: Major` is the only assertion, reaching `metadata` through the authored wiring. `#Module.metadata` carries the same holding under **D45**, which settled the half D43 left open; neither artifact type asserts the relation in `core`.
...
- `core/src/{resource,trait,blueprint,transformer}.cue`: `modulePath` retyped to `#PackagePathType`, which leaves the values every catalog ships today unchanged (D1). **`version!` is renamed `catalogVersion!` on all four kinds; a required `apiVersion!` is added to the three primitives only** (D25 as amended by D44: `core/src/transformer.cue` gets `catalogVersion!` and no `apiVersion!`, since an adapter's key is its build). The rename touches every leaf in every catalog and is mechanical. **`fqn` stops being derived** and becomes an authored value the catalog writes from its identity package (`#ContractFQNType` interpolating `apiVersion` for the first three kinds, `#ImplFQNType` interpolating `catalogVersion` for a transformer), with the agreement re-established at publish (`#CatalogMemberFQNGate`, D21, D4).
...
- `:29`, `:38` (§1 category vocabulary): **D44.** `#Blueprint` moves from **Constructs** to **Primitives**: it is a fundamental building block a component attaches and whose `spec` modules write against, notwithstanding that it composes resources and traits. `#ComponentTransformer` stays an **adapter** and `:34`'s argument for that category is unchanged and load-bearing: it is what makes `apiVersion` primitive-only. This is the one SPEC edit in the entry that changes the *taxonomy* rather than a field.
...
- `:252-256` (§3.2 `#Module` Shape): `nameSnakeCase` and `#KebabToSnake` deleted (D8), `version!` **retained** (D2) with **both halves of the identity invariant stated beside it** as D41 words them, `registryPath` added (D41), **no** version-major agreement added (D45 leaves it to the identity package, as D43 does for `#Catalog`), the `module.opmodel.dev/version` label **retained** rather than removed (D9), `modulePath!` regains the major suffix (D1), `fqn` becomes `modulePath` and `#ModuleFQNType`'s semver-with-colon form retires (D1), `uuid`'s `SHA1(OPMNamespace, fqn)` formula is unchanged but its **input changes**, so every module's UUID moves once.
...
- `:478`, `:491`, `:510`, `:520` (§3.5 `#ModuleInstance`): **the formula text changes under D41**, not only the worked values. `instance.uuid` stops deriving from `module.uuid` and derives from an explicit `instance.fqn` built on the module's `registryPath`, so the section gains a statement of why instance identity is major-free where module identity is not. `:510` and `:520`'s determinism claims stay true and get a stronger one beside them: the value survives a module major bump. All worked values move.
...
- `:605`, `:615-617`, `:650` (§4.1 `#ComponentTransformer`): `:605` "Transformers are catalog-versioned. The match algorithm is FQN-keyed" is **half-reversed**: transformers stay build-keyed (D4) while the FQNs they *demand* become contract keys. `:650` cross-references the primitive-metadata shape, and under **D44** it must stop doing so: a transformer carries `#TransformerIdentity` (`catalogVersion` but no `apiVersion`) rather than the primitive shape with one field omitted.
...
- `:658` (§4.1 Rationale) is the third statement of the union claim. It matters most because it is the justification for label-based matching: "Component labels are not authored on the Component itself: they unify upward from every attached `#Resource`, `#Trait`, and `#Blueprint` (§3.1)." Under D36 this sentence becomes true as written once `matchLabels` is substituted for `labels`. Matching labels are not authored on the component and do unify upward. Reconciled with `:216`/`:222` by the same substitution.

**library**

- `library/opm/helper/loader/internal/shape/shape.go:66`: `metadata.version` **stays** in `RequiredConcreteFields` (D2); no change.
- `library/opm/helper/loader/registry/module.go`: the module read-side check, **two clauses** (D11 + D9): the fetched artifact's `metadata.modulePath` must equal the path it was fetched by, and its `metadata.version` must equal the tag. Placing both on the `kernel.AcquireModuleFromRegistry` path means the CLI and the operator inherit one implementation. The second clause is what makes D9's schema-sourced label trustworthy, and it is the only thing standing between an artifact published by bare `cue mod publish` and a label that lies.
...
- `library/opm/kernel`: **no stamping** (D9). The label is schema-sourced; the kernel's job is the acquire-time comparison above. A stamp here would put a label's value outside the rendered CUE, where the digest gates in 0006 and 0014 cannot see how it got there.
- `library/opm/module/instance.go`, `library/opm/schema/context.go:59`: **D41.** `#TransformerContext.#moduleInstanceMetadata.fqn` is filled today with `inst.ModuleFQN()`, i.e. the *module's* FQN under an instance-shaped name. With an instance FQN defined it carries the instance's; the module's takes its own key if a reader appears. Free today: measured 2026-08-03, no shipped transformer reads `.fqn` from that block, the same 117-use survey that found no reader for `.version`.
- `library/opm/compile/module.go:137`: **must keep consuming `mp.Transformers` as read-only input from a separate resolver.** A module's primitive definitions have to resolve through the module's own dependency graph. Under D4 the failure this prevents changes shape but not severity. A single-build render would put the module and the platform in one CUE build. Minimal version selection would then pick the maximum, so the module's own copy of every contract definition would silently become the platform's. The always-unify rung is, under D26 (provenance excluded from the comparison) and D27 (the additive-only promise), the only thing checking that the module's and the provider's contract bodies agree, and with one shared resolution it would be comparing a value against itself. It could never fail. See `05-risks.md`.
...
**catalog repos** (`catalog_opm`; until D47's consolidation also `catalog_kubernetes` and `catalog_opm_experimental`: on the v2 line both fold into `catalog_opm`, as the `k8s-`-prefixed raw passthrough family with upstream-mirrored apiVersions (D48) and as `v1alpha1` abstraction members respectively, and their own v2 lines end at the `v2.0.0-alpha.1` each already published)
...
- **D42: the blueprints move up one segment.** `src/blueprints/workload/*.cue` → `src/blueprints/*.cue`, `package workload` → `package blueprints`, `modulePath` → `id.kindPrefix.blueprints`, and every blueprint FQN re-keyed (`…/blueprints/workload/stateless-workload@1.0.0` → `…/blueprints/stateless-workload@v1beta1`). `kindPrefix` admits exactly one path per kind, so no grouping subdirectory is admissible under any of them; resources, traits and transformers are already flat.

  | Repo | Import sites |
  | --- | --- |
  | `modules/` | 12 (every module in the repo): `cert_manager/components.cue:30`, `istio_ambient/components_control_plane.cue:4` and `components_dataplane.cue:4`, `jellyfin:9`, `jellystat:16`, `k8up:21`, `metallb:20`, `radarr:7`, `sabnzbd:9`, `seerr:8`, `sonarr:7`, `web_app:8` |
  | `opm-operator/test/fixtures/modules/` | 3: `hello-web/components.cue:3`, `podinfo:4`, `redis:4` (missed entirely by the original count) |
  | `cli` | 2: `tests/fixtures/modules/podinfo/components.cue:6`, `tests/integration/module-apply/testdata/components.cue:7` |
  | `library` | 2: `testdata/modules/web_app/components.cue:5`, and `opm/helper/synth/instance_integration_test.go:154` (writes the import as a string into a temp file, so it survives a grep for `import`, the one site a mechanical sweep will miss) |

  Total: 19 sites across 5 repos, re-measured 2026-08-05 against the 7 the entry originally listed.

  Five `modulePath` self-references inside the moving files change with them (`catalog_opm/src/blueprints/workload/{daemon,scheduled_task,stateful,stateless,task}_workload.cue`), and `catalog_opm/src/INDEX.md` regenerates. `core/src/blueprint.cue:17`/`:19` and `core/src/types.cue:45` carry the deep path as doc-comment examples and move too.

The undercount mattered because it made D42 read as a bounded catalog-local edit. It is not: three of the five repos holding an import site are not the catalog, so the move lands across `catalogs-identity-authoring` and the three downstream slices that own those trees. **D49 then amends the landing spot**: flat filing gave way to version-segment filing (`src/blueprints/v1beta1/`, and likewise for resources and traits), with the segment derived from each member's own `apiVersion` and never entering the FQN: D42's ban on *arbitrary* grouping stands.
- Leaf files change three lines (D21, D4, D25): `modulePath` reads `id.RegistryPath` instead of `id.ModulePath`: a one-token edit forced by `ModulePath` now carrying `@v1`; `version: id.Version` becomes `catalogVersion: id.Version`; and each primitive authors its own `apiVersion` plus an `fqn` interpolating it, `fqn: "\(id.RegistryPath)/<kind>/\(name)@\(apiVersion)"` for a contract and `…@\(id.Version)` for a transformer. `apiVersion` is the one value that is **not** derived from the identity package, but D34 makes it mechanical anyway by settling it per catalog rather than per primitive: `v1beta1` for every contract in `catalog_opm` (38) and `catalog_kubernetes` (27), `v1alpha1` for `catalog_opm_experimental`'s 3, amended by D47/D48 once the catalogs consolidate: the abstraction family keeps `v1beta1` (ex-experimental members `v1alpha1`), while the absorbed raw `k8s-*` family is assigned per member, mirroring the upstream Kubernetes API version at adoption. Transformers take none. Import statements are untouched: intra-module imports omit the major suffix (verified: `catalog_opm/cue.mod/module.cue` is `opmodel.dev/catalogs/opm@v1` while every leaf imports `opmodel.dev/catalogs/opm/identity` with no suffix), so a major bump churns no import.

**modules**

- Each module gains an `identity/` subpackage exporting `ModulePath` **and** `Version` (D2), and its root package wires `metadata: {modulePath: id.ModulePath, version: id.Version}`: the derivation `opm module init` templates and 0011 D12's publish check covers. The version is **retained**, not dropped (D2), and D41 is what makes that safe.
...
After, `modules/jellyfin/identity/identity.cue` (a subpackage, as a catalog's is, D2) holds two authored values and derives the third:

```cue
// written by opm module publish / opm module version set
ModulePath:   "opmodel.dev/modules/jellyfin@v2"
Version:      "2.4.1"
VersionMajor: "v" + strings.SplitN(Version, ".", 2)[0] // == Major (D40)
```

`module.cue` declares `name: "jellyfin"` and wires `modulePath` and `version` to those constants: a derivation `opm module init` templates, which CUE enforces for free while it is written and 0011 D12's publish check covers when an author replaces it with a literal.

`fqn` is the path. **Two identities follow from it, and they move differently** (D41):

- `module.uuid`: *which module is this*. Stable across every 2.x release, and it **moves at v3**, because `@v2` and `@v3` are distinct modules under both CUE and Go semantics.
- `instance.uuid`: *which resources does this manage*, and the value carried in the `module-instance.opmodel.dev/uuid` owner label that `prune.go:107` reads. Derived from the module's major-free `registryPath`, so it **survives the v3 bump the line above does not**. Before D41 it moved with the major and every upgrade across one silently orphaned whatever it removed.

The tag is asserted equal to `Version` at publish (0011 D12) and re-checked against it at acquire (D9), which is what lets `module.opmodel.dev/version` come from the declared value on both render paths rather than from a coordinate that exists on only one.
