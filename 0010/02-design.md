...
- **D42 — the blueprints move up one segment.** `src/blueprints/workload/*.cue` → `src/blueprints/*.cue`, `package workload` → `package blueprints`, `modulePath` → `id.kindPrefix.blueprints`, and every blueprint FQN re-keyed (`…/blueprints/workload/stateless-workload@1.0.0` → `…/blueprints/stateless-workload@v1beta1`). `kindPrefix` admits exactly one path per kind, so no grouping subdirectory is admissible under any of them; resources, traits and transformers are already flat. Seven import sites move with it — `modules/metallb/components.cue:20`, `modules/istio_ambient/components_control_plane.cue:4` and `components_dataplane.cue:4`, `library/testdata/modules/web_app/components.cue:5`, `cli/tests/fixtures/modules/podinfo/components.cue:6`, `cli/tests/integration/module-apply/testdata/components.cue:7`, and `library/opm/helper/synth/instance_integration_test.go:154`, which writes the import as a string. `core/src/blueprint.cue:17`/`:19` and `core/src/types.cue:45` carry the deep path as doc-comment examples and move too.
