...
- `core/src/module.cue:22` — `version!` **retained** (D38); the module's identity subpackage supplies it and neither `fqn` nor `uuid` reads it. **D40** adds the agreement check between its major and `modulePath`'s — the one relation between these two fields that no other gate covers.
...
- `core/src/catalog.cue:63` — `version!` kept; the `*"0.0.0-dev"` default removed. **D43:** no version-major agreement is asserted here — `identity/identity.cue`'s `VersionMajor: Major` is the only assertion, reaching `metadata` through the authored wiring. `#Module.metadata` keeps its own (D40); D43 leaves that half open.
...
- **D42 — the blueprints move up one segment.** `src/blueprints/workload/*.cue` → `src/blueprints/*.cue`, `package workload` → `package blueprints`, `modulePath` → `id.kindPrefix.blueprints`, and every blueprint FQN re-keyed (`…/blueprints/workload/stateless-workload@1.0.0` → `…/blueprints/stateless-workload@v1beta1`). `kindPrefix` admits exactly one path per kind, so no grouping subdirectory is admissible under any of them; resources, traits and transformers are already flat. **Nineteen import sites move with it, across five repos** — re-measured 2026-08-05, against the seven this entry listed until then. `modules/` carries **twelve**, which is every module in the repo: `cert_manager/components.cue:30`, `istio_ambient/components_control_plane.cue:4` and `components_dataplane.cue:4`, `jellyfin:9`, `jellystat:16`, `k8up:21`, `metallb:20`, `radarr:7`, `sabnzbd:9`, `seerr:8`, `sonarr:7`, `web_app:8`. `opm-operator/test/fixtures/modules/` carries **three** (`hello-web/components.cue:3`, `podinfo:4`, `redis:4`) and was missed entirely. `cli` carries two (`tests/fixtures/modules/podinfo/components.cue:6`, `tests/integration/module-apply/testdata/components.cue:7`) and `library` two (`testdata/modules/web_app/components.cue:5`, and `opm/helper/synth/instance_integration_test.go:154`, which **writes the import as a string** into a temp file and so survives a grep for `import` — the one site a mechanical sweep will miss). Five `modulePath` self-references inside the moving files change with them (`catalog_opm/src/blueprints/workload/{daemon,scheduled_task,stateful,stateless,task}_workload.cue`), and `catalog_opm/src/INDEX.md` regenerates. `core/src/blueprint.cue:17`/`:19` and `core/src/types.cue:45` carry the deep path as doc-comment examples and move too.

The undercount mattered because it made D42 read as a bounded catalog-local edit. It is not: three of the five repos holding an import site are not the catalog, so the move lands across `catalogs-identity-authoring` and the three downstream slices that own those trees.
...
After, `modules/jellyfin/identity/identity.cue` — a subpackage, as a catalog's is (D38, reversing D23) — holds two authored values and derives the third:
