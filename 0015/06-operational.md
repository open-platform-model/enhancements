...
**Platform-level contract readiness.** `Platform.status` gains the contract inventory's two reports. `unfulfilled` (a provider-fulfilled contract defined by a subscribed catalog that no adapter implements) is a non-gating `ContractsFulfilled=False` condition naming each contract and its defining catalog; the generated package and every render are untouched, and `Ready` keeps meaning "generated" (D18). `overSubscribed` (a provider-fulfilled contract with more than one implementation, refused per 0010 D37, kept by D2) refuses generation and is therefore a `Ready=False` reason. Both are computable on the platform value with no module in hand (under 0019 D5 as a fold over the embedded catalogs, evaluated at platform-package generation, 0019 D6), which is the whole of what D1 buys operationally: the answer moves from "the first team to deploy finds out" to "the platform says so".
...
One asymmetry is deliberate and worth knowing at the runbook level: a **refused claim does not touch its own package's readiness**: D14 excludes the registration from the wait-set precisely to break the activation deadlock, so a provider module whose claim was refused still reports Ready. The refusal surfaces only on the CR (`kubectl get transformerregistrations`, its conditions and `reason`) and in `Platform.status.registry`; checking the registration after installing a provider is the documented step, not optional. Whether the package additionally carries a non-gating condition (e.g. `RegistrationRefused=True`, never affecting Ready) is left to the operator slice's discretion: an ergonomic add, not a design requirement.
...
**Schema-side: not rollback-safe within a release, by construction.** Once `catalog_opm` is republished with populated contract maps, a kernel that predates D1 ignores them, which is harmless, since a build that skips unknown fields degrades to today's behaviour.

The reverse degrades **silently**: a kernel expecting the inventory against a catalog published without one computes an empty `defined` set, and with nothing defined, `unfulfilled` is empty by construction and the readiness answer is a vacuous `fulfilled: true`. A green `ContractsFulfilled` on a platform that cannot actually answer the question the inventory exists for. The missed-demand diagnostic loses its "defined but unimplemented" arm the same way: every miss classifies as "unknown key". So the failure to guard against is false readiness, not a wall of unfulfilled reports, and the tell is `opm platform check` showing zero defined contracts on a platform that reports `ContractsFulfilled=True`. The safe direction stands: **kernel first, catalogs second on the way forward; catalogs stay put on the way back**.
...
- `core` publishes a build carrying the new shapes, the inventory fold and the routing assertion.
- `library` publishes a kernel that surfaces them through the render build's diagnostics and the generation step.
- `catalog_opm` publishes a build whose contract maps are populated, which is what makes the readiness answer non-trivial.
- `opm-operator` consumes both and adds the CRD.

The `cli` work depends on OQ3's resolution rather than on code, so it cannot be written until that question closes.
