...
The reverse degrades **silently**: a kernel expecting the inventory against a catalog published without one computes an empty `defined` set, and with nothing defined, `unfulfilled` is empty by construction and the readiness answer is a vacuous `ready: true`. A green platform that cannot actually answer the question the inventory exists for. The missed-demand diagnostic loses its "defined but unimplemented" arm the same way: every miss classifies as "unknown key". So the failure to guard against is false readiness, not a wall of unfulfilled reports, and the tell is `opm platform check` showing zero defined contracts on a platform that claims Ready. The safe direction stands: **kernel first, catalogs second on the way forward; catalogs stay put on the way back**.
