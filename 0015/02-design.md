...
- `api/v1alpha1/platform_types.go`: `PlatformStatus` gains `registry`: the effective set (spec subscriptions + accepted claims) and a readiness condition covering unfulfilled contracts.
...
`experiments/02-three-backup-traits` renders the shape `research/backup-trait-design.md` settles on in its addendum (a `backup` policy, a `backup-producer` whose command owns its quiesce, and a catalog-fulfilled `backup-owned` projection for modules that capture themselves) for one MariaDB module and one Minecraft module on both engines with no engine-specific arm; the repository is a platform name and per-volume scope rests on a one-line PVC label in catalog_opm (P1). Provider configuration (OQ2) is the remaining obstacle.
