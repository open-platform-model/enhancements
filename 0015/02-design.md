...
Three properties fall out of it being an object rather than an event:

1. Activation is gated on `providerRef` being Ready, so a transformer never registers ahead of its CRDs: the thing a static subscription structurally cannot express. The gate latches, so once active a claim rides out provider restarts and deactivates only by deletion (D3).
2. Acceptance is centralized in the Platform reconciler, so there remains exactly one writer to the materialized set and one place to reject.
3. A finalizer can refuse deletion while instances still demand the contracts it provides, naming the count, which turns the removal hazard into a blocked delete. The same refusal covers the update door, where a provider upgrade whose re-derived `provides` drops a still-demanded contract is blocked before the new spec replaces the accepted claim (D16).

Downstream of acceptance, 0019 D6 supplies the mechanism this entry used to lack a name for: the operator regenerates the platform package the render build consumes, folding the accepted catalog's import into the registry. 0019 D8 deleted the shared materialized platform, so the regenerated package IS the effective set's materialization; the operator's per-generation record of that package survived and is held per package identity (D17). What triggers and keys the regeneration is OQ8.
...
- `core/src/catalog.cue`: `#resources`, `#traits`, `#blueprints` added beside `#transformers`, each with a `modulePath` stamping pattern constraint. The existing `#transformers` constraint is the template; the doc comment reserving these maps as "an additive extension if introspection demand surfaces later" is the line being cashed in.
...
- The D37 arity guard: rewritten as the `#ContractRouting` relation asserted on the platform value: exactly one provider per contract (0010 D37, kept by D2), with the zero case reported as `unfulfilled` on a non-gating condition (D18) and the two-provider case as `overSubscribed` naming both paths. Only over-subscription refuses at platform-package generation (0019 D6's cold path), which is where "a platform-level answer with no module in hand" naturally lives.
...
- `api/v1alpha1/platform_types.go`: `PlatformStatus` gains `registry`: the effective set (spec subscriptions + accepted claims) and a non-gating `ContractsFulfilled` condition naming unfulfilled contracts and their defining catalogs (D18); `Ready` keeps meaning "generated".
...
- Platform-package regeneration: `internal/platform/store.go` survived 0019 as the single-slot record of the generated platform module, keyed on the CR generation with a per-generation directory and a render lease. An accepted or revoked claim regenerates the package the operator builds renders from (0019 D6) under a new identity, and the record is held per identity, not per generation alone (D13, D17). What triggers regeneration, what identity the package carries, and the fleet-wide re-render blast radius are OQ8.
...
After: the contract is a member of `#catalog.#traits`, the platform's embedded copy of the catalog carries it, the inventory fold reports it, and the platform reports `ContractsFulfilled=False, reason=UnfulfilledContracts` naming `traits/backup@v1beta1` and the catalog that defines it, before any module exists, while its generated package and every render stay untouched (D18). A module that then demands `backup` fails at render naming the component and, now, the defining catalog.
...
`experiments/02-policy-and-command` renders the shape `research/backup-trait-design.md` settles on in its second addendum (a `backup` policy whose `executor` field says whether the platform or the module captures, and a `backup-command` contract whose command owns its quiesce) for one MariaDB module and one Minecraft module on both engines with no engine-specific arm; the repository is a platform name and per-volume scope rests on a one-line PVC label in catalog_opm (P1). Provider configuration (OQ2) is the remaining obstacle.
