#!/usr/bin/env bash
# Experiment 0015/02 verdict table. Run from anywhere: bash run.sh
...
PRODUCER=$P/contracts/traits/backup-producer@v1alpha1
OWNED=$P/contracts/traits/backup-owned@v1alpha1
...
run_case D k8up-only minecraft mc-demo -f "$OUT/D/consumer/values/producer.cue"
run_case E velero-only minecraft mc-demo -f "$OUT/E/consumer/values/producer.cue"
...
echo "== case B: k8up, database producer: stream, landing ignored =="
row 0 "$(cat "$OUT/B/rc")" "B: exit code"
row ok "$(python3 "$HERE/check.py" B "$OUT/B/out.json")" "B: PreBackupPod, no mounts; Schedule selects only the producer; neither PVC matches"
...
echo "== case D: k8up, world producer: PreBackupPod mounts the world beside the workload =="
...
echo "== case E: velero, SAME world module: landing pre-hook + compensate =="
...
echo "== case F: k8up, module-owned world backup: projection + maintenance only =="
...
echo "== case G: velero, SAME owned module: projection only, nothing from velero =="
...
row no "$(has "$OUT/G/stderr.txt" "is not handled by any matched transformer")" "G: an empty transformer output is not reported as an unhandled trait"
