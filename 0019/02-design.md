...
What the kernel owes that file is settled by D13: the list is derived by promotion â the platform module’s tidied list whole, the instance module’s unioned in for module-only paths, the platform winning every shared path â never computed by a render-time tidy, and the kernel refuses the render when any OPM-namespace path would resolve from the module graph rather than from the roots. It is an internal invariant rather than a policy: no caller may configure it away.
