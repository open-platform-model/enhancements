...
The full list with status lines lives in [`03-decisions.md`](03-decisions.md). In summary: OQ1 through OQ3 and OQ8 are ratified into D9; OQ4 concerns sibling-component access through `#moduleInstance`; OQ5 concerns collapsing `#TransformerContext` to a projection; OQ6 is now the load-bearing one, what the generated render module owes its own `cue.mod`; OQ7's residue is D7's default and inputs;  OQ9 and OQ10 concern enhancement 0015's runtime-discovered registrations; OQ11 concerns what publishing a platform would mean; OQ12 and OQ13 are experiment 06's residue, what a render worker may hold between renders and what replaces ADR-002's shared materialized platform; OQ14 is experiment 07's, that finalization reorders map-derived lists so the collapse emits a different `env` ordering than today's path for modules that assemble environments conditionally. Experiment 08 closes OQ12's measurement half and sharpens OQ13: per-render retention is a property of holding a `cue.Context` rather than of the collapse, and today's serialised path is 2.5x to 5.5x slower than a render worker that shares nothing.
