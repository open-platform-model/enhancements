...
- `#composedTransformers` becomes derived rather than kernel-filled: a fold over enabled entries, copying member by member rather than unifying (the catalog's provenance stamp refuses a foreign member). `#matchers` loses its filler for the same reason; whether it stays on `#Platform` as a derivation or moves into the render glue under D10 is the slice's call, recorded at the field in the delta.
