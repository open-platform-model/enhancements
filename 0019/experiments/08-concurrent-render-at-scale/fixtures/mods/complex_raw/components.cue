// complex, RAW authoring.
//
// The same services as complex_bp/components.cue, attached as primitives.
//
// This is the CONTROLLED half of the style comparison. The blueprint variant
// attaches bp.#StatefulWorkload, which composes the Container and Volumes
// resources and the Scaling, RestartPolicy, UpdateStrategy, SidecarContainers
// and InitContainers traits. This module uses every one of them, so the list
// below is exactly that set and nothing is dropped or added. What remains
// different is only the blueprint wrapper itself:
//
//   - the `#blueprints` map entry, carrying the whole #StatefulWorkloadBlueprint
//     definition including its composedResources and composedTraits lists (which
//     are themselves full #Resource and #Trait definitions);
//   - the `spec.statefulWorkload` field, a second copy of every value written;
//   - the six guarded assignments that propagate that copy onto the primitive
//     spec fields the transformers actually read.
//
// The fleet pair measures the other half: a blueprint attaching traits the
// module does not use. Both must render byte-identical output; the harness
// compares digests.
package complex_raw
...
#components: {
	for _n, _s in #config.services {
		...
		(_n): {
			...
			// Answered at the ATTACHMENT SITE. res.#ContainerResource declares
			// this matching key as REQUIRED with a disjunction for a value
			// (0010 D36); core derives `matchLabels` from the attached
			// primitives and `_matchLabelsAreDerived` refuses any key the
			// component adds itself, so a raw author cannot write
			// `matchLabels:` on the component. Narrowing the resource that
			// owns the key is the only spelling that works, and answering it
			// is precisely the work a workload blueprint exists to do.
