...
- **A Go walk over the built platform value after the build.** Rejected: a second matcher-adjacent Go pass beside a glue that already folds the same map, the direction D1 names as wrong, and its verdict would not sit beside the others as data.
- **Drop enforcement until enhancement 0011's publish gate carries it.** Rejected: a silent regression of 0010 D37 with no tripwire between the cutover and 0011, and a publish-time check on one catalog cannot see which catalogs one platform composes.
- **Delete it as vacuous.** Rejected on measurement: two catalogs each shipping a provider for one key compose without conflict in one build, because `#composedTransformers` is a per-entry copy fold (D17), so nothing else refuses such a platform. `02-design.md` left this open ("whether they survive, or become vacuous"); this decision closes it.

**Rationale:** The row costs one comprehension over a map the glue already holds, the refusal keeps naming the key and both registry keys as the materialize-era error did, and verdicts-as-data is the shape every other gate cause already takes. Measured 2026-09-03 on `library` (openspec change `library-render-cutover`, fixtures `testdata/render/platform_oversubscribed` and `platform_two`): the over-subscribed platform refuses with the row decoded beside intact pair verdicts, and two catalogs supplying a catalog-fulfilled contract render with every candidate matched. One CUE scoping hazard is recorded for anyone editing the glue: a field spelled `overSubscribed: overSubscribed` inside `diagnostics` refers to itself and evaluates to top while the gate still refuses, so the fold lives under its own `guard` struct and is read as `guard.overSubscribed`.

**Source:** User decision 2026-09-03 ("go with 1": the in-build row, recommendation accepted as presented; recorded here at the user's request). Evidence: `library/opm/internal/renderstage/render.cue.tmpl`, `library/opm/kernel/render_test.go` (`TestRender_OverSubscribedProviderRefused`, `TestRender_CatalogFulfilledPluralityRenders`).
