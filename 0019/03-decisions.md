...
**Rationale:** The three types are not an OPM taxonomy; they are transcriptions of the server's three validators (subdomain for most `metadata.name`, DNS-1035 for Service, label composition for the DNS-bearing kinds), verified by server-side dry-run: dots accepted on Deployment/DaemonSet/ConfigMap/StorageClass/CSIDriver, refused on Service ("DNS-1035 label"), StatefulSet and Namespace ("must not contain dots"). A name is dot-restricted iff it becomes a DNS label, and the type system now says so.
