# Delivery Plan — Module and Catalog Publishing
...
graph LR
  ...
  X_artifact-identity_core-identity-shape["artifact-identity:core-identity-shape"]:::other
  X_artifact-identity_core-identity-shape -->|depends_on| S_core-identity-package
  S_core-identity-package -->|depends_on| S_cli-publish-pipeline
  X_artifact-identity_cli-coordinate-adoption["artifact-identity:cli-coordinate-adoption"]:::other
  X_artifact-identity_cli-coordinate-adoption -->|depends_on| S_cli-publish-pipeline
  ...
  X_artifact-identity_catalogs-identity-authoring["artifact-identity:catalogs-identity-authoring"]:::other
  X_artifact-identity_catalogs-identity-authoring -->|depends_on| S_catalogs-publish-cutover
  ...
  X_artifact-identity_modules-identity-authoring["artifact-identity:modules-identity-authoring"]:::other
  X_artifact-identity_modules-identity-authoring -->|depends_on| S_modules-publish-cutover
  S_registry-cleanup -->|depends_on| S_registry-cleanup-operator
```

| ID | Phase | Repo | Status | Depends on | Concern |
| -- | ----- | ---- | ------ | ---------- | ------- |
| core-identity-package | implementation | core | done | artifact-identity:core-identity-shape | Ship #IdentityPackage and #CatalogMemberFQNGate in core so publish validates identity and every catalog member by unification and CUE produces the diagnostic, not a hand-rolled comparison.   |
...
| cli-publish-pipeline | implementation | cli | done | core-identity-package, artifact-identity:cli-coordinate-adoption | One pipeline, two entry points: decode, read identity, derive coordinates, run the gates, push, with the dry-run plan output. Refusals 1-8 and 10, plus the D16/D18 checks in `opm module vet`.   |
...
| catalogs-publish-cutover | implementation | catalog | done | cli-publish-pipeline, cli-authoring-commands, cli-catalog-gates, cli-login, artifact-identity:catalogs-identity-authoring | Switch the consolidated catalog_opm's release.yml to `opm catalog publish`, delete the copy-and-stamp task — one catalog repo since the consolidation. Catalogs go first because modules build against them.   |
| modules-publish-cutover | implementation | modules | done | cli-publish-pipeline, cli-authoring-commands, catalogs-publish-cutover, artifact-identity:modules-identity-authoring | Delete the checksum-driven publish and versions.yml, cut over to `opm module publish`. The identity file itself is 0010's modules-identity- authoring; coordinates do not change.   |
